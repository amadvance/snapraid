...
char* u16tou8_mayfail(char* conv_buf, size_t conv_size, const wchar_t* src, size_t number_of_wchar, size_t* result_length_without_terminator)
{
	...
	int ret = WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, src, number_of_wchar, conv_buf, conv_size, 0, 0);
	if (ret <= 0)
		return 0;
...
char* u16tou8(char* conv_buf, const wchar_t* src)
{
	...
	return u16tou8ex(conv_buf, CONV_MAX, src, wcslen(src) + 1, 0);
}

static char* u16tou8size(char* conv_buf, size_t conv_size, const wchar_t* src)
{
	/* convert also the 0 terminator */
	return u16tou8ex(conv_buf, conv_size, src, wcslen(src) + 1, 0);
}
...
static int windows_stream2dirent(const BY_HANDLE_FILE_INFORMATION* info, const FILE_ID_BOTH_DIR_INFO* stream, struct windows_dirent* dirent)
{
	...
	name = u16tou8ex(conv_buf, CONV_MAX, stream->FileName, stream->FileNameLength / 2, &len);
...
int windows_readlink(const char* file, char* buffer, size_t size)
{
	...
	name = u16tou8ex(conv_buf_name, CONV_MAX,
...
pid_t os_spawn(char** argv, int* stdout_read_int, int* stderr_read_int, const char* run_as_user)
{
	...
	if (!ret) {
		windows_errno(GetLastError());
		os_syslog(OS_LVL_INFO, "failed to create process '%s' for spawn, errno=%s(%d)", u16tou8size(cmd_buffer_conv, sizeof(cmd_buffer_conv), cmd_buffer), strerror(errno), errno);
		if (has_out) {
			CloseHandle(stdout_write_handle);
			close(out_f);
		}
...
int os_script(char** argv, char** envp, const char* run_as_user)
{
	...
	if (!ret) {
		windows_errno(GetLastError());
		os_syslog(OS_LVL_INFO, "failed to create process '%s' for script, errno=%s(%d)", u16tou8size(cmd_buffer_conv, sizeof(cmd_buffer_conv), cmd_buffer), strerror(errno), errno);
		return -1;
	}
