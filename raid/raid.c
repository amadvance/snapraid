...
/*
 * Extended Cauchy Matrix Construction for MDS RAID in GF(2^8)
 *
 * The Cauchy RAID modes operate in GF(2^8) and support up to six parity
 * levels. RAID_MODE_CAUCHY_RAID uses polynomial 0x11d, while
 * RAID_MODE_CAUCHY_AES uses polynomial 0x11b.
 *
 ...
 *     X[i] = inv(q[i])
 *
 * The Cauchy Y set starts with zero and consumes inverses backward from the
 * end of the sequence:
 *
 *     Y[0] = 0
 *     Y[j] = inv(q[255-j]) for j > 0
 *
 * Thus the Y values associated with R, S, T, and U are inv(q[254]),
 * inv(q[253]), inv(q[252]), and inv(q[251]), respectively. For PARITY=n and
 * DISK=257-n, increasing parity removes one X element from the tail and
 * appends it to Y, leaving the existing rows unchanged on the remaining
 * columns. X and Y are disjoint and together contain all 256 field elements, so
 * C[j,i]=1/(X[i]+Y[j]) defines valid Cauchy rows. Adding P as an all-ones row
 * forms an Extended Cauchy matrix. Q is the row for Y[0]=0 and therefore
 * Q=sum(q[i]*Di). Rows R through U are scaled so their first coefficient is 1.
 * This nonzero row scaling preserves the MDS property, and the resulting 6x251
 * matrix is MDS.
 *
 ...
 *
 * The RAID/g=2 construction can be extended beyond six parity levels by
 * adding rows and removing one data column for each additional row, while
 * preserving the field-size constraint i + j < 255. The current
 * implementation remains limited to RAID_PARITY_MAX = 6.
