...
/*
 * Initialize the countdown used by the AES G23 Horner recurrence.
 *
 ...
 *
 * Rather than comparing d against those four values in the hot loop, we keep
 * a countdown. Before processing each Horner step the generator does:
 *
 *     if (--g23_count == 0) {
 *         Q = 3 * Q;
 *         g23_count = 51;
 ...
 *     }
 *
 * This function returns the initial countdown for a traversal whose first
 * processed step has index d. For d >= 50, (d - 50) % 51 gives the distance
 * from the previous x3 boundary, so adding 1 gives the number of decrements
 * required before the next descending x3 transition is reached.
 *
 * For d < 50 there is no remaining x3 transition. Returning 51 is safe
 * because fewer than 51 Horner steps remain, so the countdown cannot reach
 * zero before the traversal ends.
 *
 * Examples:
 *
 *     d = 203 ->  1  -> x3 immediately
 *     d = 204 ->  2  -> x2 at 204, x3 at 203
 *     d = 250 -> 48  -> x3 at 203
 *     d = 152 ->  1  -> x3 immediately
 *     d =  50 ->  1  -> x3 immediately
 *     d =  49 -> 51  -> no further x3 transition
 *
 * The returned value is normally computed once as g23_start and copied into
 * g23_count at the beginning of each independent Horner traversal.
 */
