...
/*
 * Check if the AES G23 Q recurrence contains only x2 transitions
 * for the specified number of data disks.
 */
static __always_inline int raid_g23_x2_only(int nd)
{
	return nd <= 51;
}

/*
 * Find the next AES G23 x3 boundary in descending Horner order.
 *
 * The G23 Q coefficients are generated by multiplying by 2 at every step,
 * except at the four boundaries between the five cosets, where the next
 * coefficient is obtained by multiplying by 3 instead:
 *
 *     q[i + 1] = 3 * q[i]   for i = 50, 101, 152, 203
 *     q[i + 1] = 2 * q[i]   otherwise
 *
 * The optimized parity generators evaluate Q in descending Horner order.
 * If the current step is identified by disk index d, the x3 transitions are
 * therefore encountered at:
 *
 *     d = 203, 152, 101, 50
 *
 * i.e. every 51 descending steps.
 *
 * The generators can process all the x2 transitions between two consecutive
 * boundaries in a branch-free loop, and handle the x3 transition separately.
 * This function returns the first x3 boundary encountered by a descending
 * traversal whose first processed step has index d.
 *
 * For d >= 50, (d - 50) % 51 gives the distance from the previous x3
 * boundary in ascending index order. Subtracting this distance from d gives
 * that boundary directly:
 *
 *     boundary = d - (d - 50) % 51
 *
 * Since the Horner traversal proceeds toward decreasing indexes, this is also
 * the next x3 transition that will be encountered.
 *
 * For d < 50 there is no remaining x3 transition, so -1 is returned. This
 * value can also be used as the lower bound of the final x2-only segment,
 * allowing all remaining steps down to d = 0 to be processed without a
 * special G23 test.
 *
 * Examples:
 *
 *     d = 249 -> 203 -> x2 down to 204, then x3 at 203
 *     d = 203 -> 203 -> x3 immediately
 *     d = 202 -> 152 -> x2 down to 153, then x3 at 152
 *     d = 152 -> 152 -> x3 immediately
 *     d = 151 -> 101 -> x2 down to 102, then x3 at 101
 *     d = 101 -> 101 -> x3 immediately
 *     d = 100 ->  50 -> x2 down to 51, then x3 at 50
 *     d =  50 ->  50 -> x3 immediately
 *     d =  49 ->  -1 -> no further x3 transition
 *
 * After processing a boundary, the following one in descending order is
 * exactly 51 positions lower. Thus generators can continue with:
 *
 *     boundary -= 51;
 *     if (boundary < 50)
 *         boundary = -1;
 *
 * The returned value is normally computed once before each independent
 * Horner traversal and then advanced between the fixed G23 segments.
 */
static __always_inline int raid_g23_boundary(int d)
{
	if (d < 50)
		return -1;
