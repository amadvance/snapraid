...
Motif (Pattern)
	Les motifs sont utilisés pour sélectionner un sous-ensemble de fichiers
	à exclure ou à inclure dans le processus.

	Il existe quatre types de motifs différents :

	=FICHIER
		Sélectionne tout fichier nommé FICHIER. Vous pouvez utiliser
		tous les caractères de globbing comme * et ?, et les classes de
		caractères comme [a-z].
		Ce motif s'applique uniquement aux fichiers, pas aux répertoires.

	=DOSSIER/
		Sélectionne tout répertoire nommé DOSSIER et tout ce qu'il
		contient. Vous pouvez utiliser tous les caractères de globbing
		comme * et ?.
		Ce motif s'applique uniquement aux répertoires, pas aux fichiers.

	=/CHEMIN/FICHIER
		Sélectionne le chemin de fichier exact spécifié. Vous pouvez
		utiliser tous les caractères de globbing comme * et ?, mais
		ils ne correspondent jamais à une barre oblique de répertoire.
		Ce motif s'applique uniquement aux fichiers, pas aux répertoires.

	=/CHEMIN/DOSSIER/
		Sélectionne le chemin de répertoire exact spécifié et tout ce qu'il
		contient. Vous pouvez utiliser tous les caractères de globbing
		comme * et ?, mais ils ne correspondent jamais à une barre
		oblique de répertoire.
		Ce motif s'applique uniquement aux répertoires, pas aux fichiers.
