...
Einschränkungen (Limitations)
	...
	Die Haupteinschränkung besteht darin, dass Sie bei einem Festplattenausfall Daten nur bis zum
	Zustand des letzten `sync`-Vorgangs wiederherstellen können. Alle Daten, die seit dem
	letzten Sync hinzugefügt oder geändert wurden und sich auf der ausgefallenen Festplatte befinden, gehen verloren.

	Daher eignet sich SnapRAID in erster Linie für Daten, die sich selten ändern.

	Für Daten, die beim letzten Sync bereits existierten, hängt die Wiederherstellungszuverlässigkeit
	...
	* Ohne Snapshot-Unterstützung kann das Löschen oder Ändern von Dateien nach einem `sync`
		die vollständige Wiederherstellung anderer ausgefallener Festplatten verhindern. Dies geschieht,
		weil die Parität nicht mehr mit den geänderten Dateien übereinstimmt, selbst wenn
		sich diese Dateien nicht auf der ausgefallenen Festplatte befinden. Neu hinzugefügte
		Dateien verhindern dagegen die Wiederherstellung bereits vorhandener Dateien nicht.
...
Befehle (Commands)
	...
	* Einen Backup-/Wiederherstellungspunkt erstellen -> `sync`
	* Daten regelmäßig überprüfen -> `scrub`
	* Den letzten Backup-/Wiederherstellungspunkt wiederherstellen -> `fix`.
  ...
  fix
	Repariert alle Dateien und die Paritätsdaten.

	Alle Dateien und Paritätsdaten werden mit dem im letzten `sync`
	gespeicherten Zustand verglichen. Wenn ein Unterschied gefunden
	wird, wird er auf den gespeicherten Zustand zurückgesetzt.

	WARNUNG!
	Der Befehl `fix` unterscheidet nicht zwischen Fehlern und
...
Konfiguration (Configuration)
  ...
  snapshot
	...
	Paritätsfestplatten oder Datenträger mit anderen Dateisystemen verwenden immer
  ...
  exclude/include PATTERN
	...
	Beachten Sie, dass die Filterung hierarchisch beim Durchlaufen der
	Verzeichnisse erfolgt. Wenn ein Verzeichnis mit einer `exclude`-Regel
	übereinstimmt, wird das gesamte Verzeichnis übersprungen (verworfen),
	und alle `include`-Regeln für darin enthaltene Dateien oder
	Unterverzeichnisse werden nicht ausgewertet.
...
Snapshots
	...
	durch Snapshots vollständig im Hintergrund abgewickelt wird.
	...
	Wiederherstellung - Wenn eine Datei im Live-Dateisystem aktualisiert oder gelöscht wird,
		...
		um die ausgefallene Festplatte zu rekonstruieren.

		Im Falle eines Festplattenausfalls während eines aktiven Sync-Vorgangs
		ist SnapRAID auch in der Lage, automatisch sowohl aus dem Snapshot
		der vorherigen Paritätsberechnung als auch aus der aktuellen zu lesen.
		Dies maximiert die Wahrscheinlichkeit einer vollständigen Wiederherstellung, indem
		Zugriff auf die genauen Datenblöcke gewährt wird, die zur Lösung der Paritätsgleichungen
		erforderlich sind, selbst wenn diese Blöcke zwischen den Syncs geändert oder gelöscht wurden.

		Ohne Snapshots führt eine aktualisierte oder gelöschte Datei auf einer gesunden
		Festplatte zu fehlenden Datenblöcken, die möglicherweise zur Reparatur
		anderer ausgefallener Festplatten benötigt werden.
	...
	Sie können Datenträger mit unterschiedlichen Dateisystemen mischen. Nur diejenigen, die
	Snapshots unterstützen, werden diese nutzen, während andere Festplatten weiterhin
	direkt auf dem Live-Dateisystem arbeiten.
  ...
  Befehlsverhalten mit Snapshots (Command Behavior with Snapshots)
	...
	Bei den Befehlen `check` und `fix` hängt die Verwendung des letzten Snapshots
	davon ab, ob bestimmte Festplatten mit der Option -d, --filter-disk ausgewählt wurden.

	Eine explizit über -d ausgewählte Festplatte (das „Ziel“ der Operation)
	verwendet immer das Live-Dateisystem. Für `fix` ermöglicht dies das Wiederherstellen
	von Daten auf dem aktiven Festplattenersatz. Für `check` ermöglicht es die Simulation
	des `fix`-Vorgangs unter denselben Bedingungen.

	Auf alle anderen Datenträger (die „Referenz“-Festplatten) wird über ihre Snapshots zugegriffen.
	Dies stellt sicher, dass SnapRAID selbst dann, wenn Sie während einer laufenden Wiederherstellung
	Dateien auf Ihren gesunden Festplatten ändern, eine stabile, eingefrorene Referenz hat,
	um die Paritätsgleichungen zu lösen.

	Wenn keine Option -d angegeben ist, nimmt SnapRAID an, dass die Operation für das gesamte Array gilt.
	In diesem Fall verwenden `check` und `fix` ausschließlich die Live-Dateisysteme.
...
Lebenszyklus von Snapshots (Snapshots Lifecycle)
	...
	Der `stable`-Snapshot repräsentiert den Zustand des letzten
	erfolgreich abgeschlossenen `sync` und enthält die genauen Daten, die zur
	...
	Wenn ein `sync` unterbrochen wird, bleibt der `pending`-Snapshot erhalten.
	Nachfolgende `scrub`-, `check`- und `fix`-Befehle verwenden diesen ausstehenden Snapshot,
	da er mit dem in der .content-Datei aufgezeichneten Dateisystemzustand übereinstimmt,
	selbst wenn die Parität nur teilweise synchronisiert ist.

	Im Falle einer `sync`-Unterbrechung lesen die Befehle `check` und `fix`
	auch aus dem `stable`-Snapshot, um Datenblöcke von Dateien abzurufen,
	die während des Syncs aktualisiert oder gelöscht wurden.
	Dies ist möglich, da die .content-Datei Metadaten für alle geänderten oder
	gelöschten Dateien während des gesamten Sync-Prozesses aufbewahrt.
	Diese historischen Informationen werden erst gelöscht, wenn ein Sync erfolgreich abgeschlossen
	und die endgültige Version der .content-Datei gespeichert wurde.
...
Muster (Pattern)
	...
	Der Doppelstern-Platzhalter `**` entspricht nur dann über Verzeichnistrennzeichen
	hinweg, wenn er an ein Verzeichnistrennzeichen angrenzt (einem Schrägstrich
	vorangestellt oder nachgestellt). In dieser Form entspricht er null oder mehr
	Zeichen über Verzeichnisebenen hinweg. Zum Beispiel entspricht `foo/**/bar` den
	Pfaden `foo/bar`, `foo/a/bar` und `foo/a/b/bar`; `**/build` entspricht `build`
	und `foo/bar/build`; und `src/**.js` entspricht `src/main.js` und
	`src/ui/components/main.js`.
	Die alleinstehende Form `/**/` wird nicht als vollständiges Muster akzeptiert.

	Wenn `**` nicht an einen Schrägstrich angrenzt, verhält er sich wie `*` und
	entspricht keinen Verzeichnistrennzeichen. Zum Beispiel entspricht `**.jpg`
	der Datei `photo.jpg` im aktuellen Verzeichnis, nicht jedoch `dir/photo.jpg`.
	Verwenden Sie für diesen Zweck `**/*.jpg`.
	...
	übereinstimmen, während `**` nur dann über Verzeichnistrennzeichen hinweg übereinstimmen kann,
	wenn es an einen Schrägstrich angrenzt.
	...
	Beachten Sie, dass das Ausschließen eines Verzeichnisses (mit DIR/
	oder /PFAD/DIR/) verhindert, dass SnapRAID dieses Verzeichnis betritt
	und durchsucht. Daher ist es nicht möglich, eine Datei oder ein
	Unterverzeichnis innerhalb eines ausgeschlossenen übergeordneten
	Verzeichnisses wieder einzuschließen, da der Scanner dieses niemals
	durchläuft.

	Beispielsweise wird bei folgender Konfiguration `file.dat` NICHT
	eingeschlossen, da `/keep/` verworfen wird, bevor `file.dat` gesehen
	werden kann:

		:# Funktioniert NICHT: /keep/ wird vollständig übersprungen
		:include /keep/file.dat
		:exclude /keep/

	Um Dateien innerhalb eines Verzeichnisses auszuschließen und bestimmte
	Dateien beizubehalten, schließen Sie die Dateien im Verzeichnis aus,
	anstatt das Verzeichnis selbst auszuschließen:

		:# Funktioniert: /keep/ wird durchlaufen, aber andere Dateien darin werden ausgeschlossen
		:include /keep/file.dat
		:exclude /keep/*
...
Inhalt (Content)
	...
	Die Content-Datei wird von den Befehlen `sync`, `scrub`, `touch` und
	`rehash` gelesen und geschrieben. Sie wird von den Befehlen `status`,
	`diff`, `check`, `fix`, `list`, `dup`, `locate` und `pool` gelesen und
...
Bekannte Probleme (Known Issues)
  ...
  Nicht-ASCII-Groß-/Kleinschreibung unter Windows
	SnapRAID verwendet auf allen Plattformen eine standardmäßige, vorhersehbare
	UTF-8-Kodierung und beschränkt den Vergleich ohne Berücksichtigung der
	Groß-/Kleinschreibung auf ASCII-Zeichen.

	Windows-Dateisysteme leiden unter dem veralteten Konzept, die Unempfindlichkeit
	gegenüber der Groß-/Kleinschreibung bei Unicode-Dateinamen über willkürliche,
	systemabhängige Folding-Tabellen zu erzwingen. Infolgedessen verschleiert Windows
	die Realität des Dateisystems, indem es völlig unterschiedliche
	Unicode-Bytesequenzen so behandelt, als wären sie dieselbe Datei.

	Beispielsweise sind `À.txt` und `à.txt` unterschiedliche Unicode-Namen, dennoch
	behandelt Windows sie unlogischerweise als identisch, während SnapRAID ihre
	tatsächliche Unterscheidung beibehält.

	Diese Einschränkung wird durch den inkonsistenten Umgang von Windows mit
	Unicode-Dateinamen und das Beharren auf einer Groß-/Kleinschreibungs-Aliasing
	verursacht. Um Abweichungen durch das Case-Folding von Windows zu vermeiden,
  ...
  Reservierte Datei- und Verzeichnisnamen
	SnapRAID reserviert einige Datei-, Verzeichnis- und Snapshot-Namen für seine
	internen Operationen. Verwenden Sie diese Namen nicht für Benutzerdaten:

	.snapraidignore - Lokale Ausschlussregeldatei.
	.snapraid - Snapshot-Container-Verzeichnis.
	snapraid-* - Von SnapRAID verwaltete ZFS-Snapshot-Namen.
	*.unrecoverable - Beschädigte Dateien, die während der Wiederherstellung
		unter Quarantäne gestellt wurden.
	CONTENT.tmp - Temporäre Datei für Content-Aktualisierungen.
	CONTENT.lock - Content-Sperrdatei.
	PARITY.spaceholder - Temporäre Platzhalterdatei während der
		Paritätszuweisung unter Windows.

	Die Verwendung dieser Namen für nicht zugehörige Daten kann dazu führen,
	dass Dateien oder Snapshots ignoriert, als Metadaten fehlinterpretiert,
	überschrieben, umbenannt oder gelöscht werden.

	Unter Windows sind alle Varianten der Groß- und Kleinschreibung ebenfalls
	reserviert, selbst in NTFS-Verzeichnissen, die für die Unterscheidung von
	Groß- und Kleinschreibung konfiguriert sind (z. B. .SNAPRAIDIGNORE,
	.SnapRaid, CONTENT.TMP und CONTENT.Lock).
