...
Thermal Tags
	...
	=thermal:current:<disk_name>[/<split_number>]:<device>:<temp>
	...
	=thermal:heat:<disk_name>[/<split_number>]:<device>:<count>:<data_points>
	...
	=thermal:params:<disk_name>[/<split_number>]:<device>:<k_heat>:<t_ambient>:<t_steady>:<rmse>:<r_squared>:<max_error>
...
Command Smart Tags
	...
	=attr:<device_file>:<disk_name>[/<split_number>]:vendor:<vendor>
		Logs the disk's vendor string.

		<vendor> - The disk's vendor identifier (escaped).

	=attr:<device_file>:<disk_name>[/<split_number>]:model:<model>
		Logs the disk's model string.

		<model> - The disk's model identifier (escaped).

	=attr:<device_file>:<disk_name>[/<split_number>]:afr:<afr_value>:<afr_prob>
	...
	=attr:<device_file>:<disk_name>[/<split_number>]:size:<size_bytes>
		Logs the size of the disk.

		<size_bytes> - The disk size in bytes (uint64).

	=attr:<device_file>:<disk_name>[/<split_number>]:error:<error_count>
		Logs the total error count.

		<error_count> - The total error count (uint64).

	=attr:<device_file>:<disk_name>[/<split_number>]:rotationrate:<rate>
		Logs the disk's rotation rate. A value of 0 indicates an SSD.

		<rate> - The rotation rate (uint64, RPM).

	=attr:<device_file>:<disk_name>[/<split_number>]:flags:<flags_decimal>:<flags_hex>
	...
	=attr:<device_file>:<disk_name>[/<split_number>]:<id>:<value_decimal>:<value_hex>
