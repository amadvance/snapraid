...
Description
	...
	In GUI mode, the structured log is intended to be consumed by the GUI.
	Fatal errors are therefore not duplicated to standard error when they
	are successfully written to the log. If the log is unavailable or
	writing it fails, the fatal error is printed to standard error as a
	fallback.
...
Error Tags
	...
	=error[_io|_data]:<block>:<disk_name>:<file>:<msg>
		Logs errors affecting data files.

		With no suffix, `error` indicates a soft or logical error not
		classified as a hardware I/O or silent data error.

		_io - Indicates a physical Input/Output error, typically
			signaling a failing sector, device, or cable issue.
		_data - Indicates data that was read successfully but failed
			the checksum/hash verification, signaling `silent`
			corruption or an unsuccessful parity match.
		<block> - The zero-based SnapRAID parity position associated with
			the error (uint). It identifies the logical position affected
			by the failed operation, not necessarily the physical sector
			that caused the failure. Data file open/create errors occurring
			while processing a position report the current block. The field
			is empty when the error cannot be associated with a specific
			position, such as for close errors.
	...
	=parity_error[_io|_data]:<block>:<level>:<msg>
		Logs errors affecting parity.

		With no suffix, `parity_error` indicates a soft or logical error
		not classified as a hardware I/O or silent data error.

		_io - Indicates a physical Input/Output error, typically
			signaling a failing sector, device, or cable issue.
		_data - Indicates data that was read successfully but failed
			the parity verification, signaling `silent` corruption.
		<block> - The zero-based SnapRAID parity position associated with
			the error (uint). Parity open/create errors report block 0 as
			a retry marker; this does not mean that parity block 0 is bad.
			Parity access is attempted again by subsequent commands. The
			field is empty when the error cannot be associated with a
			specific position, such as for close errors.
