...
Pattern
	Wzorce są używane do wybierania podzbioru plików do wykluczenia lub włączenia
	do procesu.

	Istnieją cztery różne typy wzorców:

	=FILE
		Wybiera dowolny plik o nazwie FILE. Możesz użyć dowolnych
		znaków globbing, takich jak * i ?, oraz klas znaków, takich jak [a-z].
		Ten wzorzec dotyczy tylko plików, a nie katalogów.

	=DIR/
		Wybiera dowolny katalog o nazwie DIR i wszystko w środku.
		Możesz użyć dowolnych znaków globbing, takich jak * i ?.
		Ten wzorzec dotyczy tylko katalogów, a nie plików.

	=/PATH/FILE
		Wybiera dokładnie określoną ścieżkę pliku. Możesz użyć dowolnych
		znaków globbing, takich jak * i ?, ale nigdy nie pasują one do
		ukośnika katalogu.
		Ten wzorzec dotyczy tylko plików, a nie katalogów.

	=/PATH/DIR/
		Wybiera dokładnie określoną ścieżkę katalogu i wszystko
		w środku. Możesz użyć dowolnych znaków globbing, takich jak * i ?, ale
		nigdy nie pasują one do ukośnika katalogu.
		Ten wzorzec dotyczy tylko katalogów, a nie plików.
