...
Polecenia (Commands)
  ...
  diff
	...
	podsumowanie zmian, pogrupowane według:
		...
		moved - Pliki przeniesione lub o zmienionej nazwie na tym samym dysku
			z zachowaniem tego samego i-węzła (inode). Są identyfikowane przez
			ten sam inode, rozmiar i znacznik czasu, ale inną ścieżkę. Nazwa
			pliku może być również inna.
		copied - Pliki skopiowane na ten sam lub inny dysk, gdzie
			...
			tylko nazwa.
		relocated - Pliki przeniesione do innego katalogu lub na inny dysk,
			gdzie oryginalny plik zniknął, a inode nie został zachowany.
			Są identyfikowane przez posiadanie tej samej nazwy, rozmiaru i
			znacznika czasu. Jeśli podsekundowy znacznik czasu wynosi zero,
			pełna ścieżka musi być zgodna, aby plik został zidentyfikowany.
			W przeciwieństwie do plików 'moved', pliki relokowane wymagają,
			aby nazwa pliku pozostała taka sama.
...
Konfiguracja (Configuration)
  ...
  exclude/include PATTERN
	...
	Należy pamiętać, że filtrowanie odbywa się hierarchicznie podczas
	przechodzenia katalogów. Jeśli katalog pasuje do reguły `exclude`, cały
	katalog jest pomijany (odrzucany), a wszelkie reguły `include` dla plików
	lub podkatalogów w nim zawartych nie będą oceniane.
  ...
  smartctl DISK/PARITY OPTIONS...
	...
	Możesz również określić niestandardowy ciąg opcji informacyjnych za pomocą
	znacznika `[info: ...]`, aby zastąpić domyślną opcję `-a`.
...
Wzorce (Pattern)
	...
	Symbol wieloznaczny podwójnej gwiazdki `**` dopasowuje się poprzez separatory
	katalogów tylko wtedy, gdy sąsiaduje z separatorem katalogów (jest poprzedzony
	lub zakończony ukośnikiem). W tej formie dopasowuje zero lub więcej znaków
	na wielu poziomach katalogów. Na przykład `foo/**/bar` dopasowuje `foo/bar`,
	`foo/a/bar` oraz `foo/a/b/bar`; `**/build` dopasowuje `build` oraz
	`foo/bar/build`; a `src/**.js` dopasowuje `src/main.js` oraz
	`src/ui/components/main.js`.
	Samodzielna forma `/**/` nie jest akceptowana jako kompletny wzorzec.

	Gdy `**` nie sąsiaduje z ukośnikiem, zachowuje się jak `*` i nie dopasowuje
	separatorów katalogów. Na przykład `**.jpg` dopasowuje `photo.jpg` w bieżącym
	katalogu, ale nie pasuje do `dir/photo.jpg`. W tym celu należy użyć `**/*.jpg`.
	...
	składnika ścieżki, podczas gdy `**` może dopasowywać się poprzez separatory
	katalogów tylko wtedy, gdy sąsiaduje z ukośnikiem.
	...
	Należy pamiętać, że wykluczenie katalogu (za pomocą DIR/ lub /PATH/DIR/)
	uniemożliwia programowi SnapRAID wejście i skanowanie tego katalogu.
	W konsekwencji nie jest możliwe ponowne włączenie pliku lub podkatalogu
	znajdującego się w wykluczonym katalogu nadrzędnym, ponieważ skaner nigdy
	do niego nie przechodzi.

	Na przykład poniższa konfiguracja NIE uwzględni pliku `file.dat`,
	ponieważ katalog `/keep/` zostanie odrzucony, zanim `file.dat` będzie
	mógł zostać sprawdzony:

		:# NIE działa zgodnie z oczekiwaniami: /keep/ jest całkowicie pomijany
		:include /keep/file.dat
		:exclude /keep/

	Aby wykluczyć pliki wewnątrz katalogu, zachowując wybrane, należy dopasować
	wzorzec plików wewnątrz katalogu, zamiast wykluczać sam katalog:

		:# Działa: /keep/ jest przeszukiwany, ale pozostałe pliki są wykluczane
		:include /keep/file.dat
		:exclude /keep/*
...
Znane problemy (Known Issues)
  ...
  Odzyskiwanie po przerwanym sync z wieloma parzystościami
	W macierzy z wieloma dyskami parzystości przerwane polecenie `sync` może
	pozostawić różne poziomy parzystości chwilowo niezsynchronizowane ze sobą.
	Na przykład parzystość 1 może już zawierać nowo zaktualizowane dane,
	podczas gdy parzystość 2 nadal zawiera stare dane.

	Podczas wykonywania `check` lub `fix` SnapRAID sprawdza różne kombinacje
	parzystości i weryfikuje zrekonstruowane dane z wykorzystaniem znanych
	skrótów bloków (obejmujących cały blok, zazwyczaj 256 KiB).

	Jeśli SnapRAID podejmie próbę rekonstrukcji z użyciem niespójnej
	mieszanki starej i nowej parzystości, liniowe równania RAID dadzą
	nieprawidłowe dane. Aby taka niepoprawna kombinacja została omyłkowo
	zaakceptowana, równania musiałyby przypadkowo zrekonstruować co najmniej
	jeden pełny blok 256 KiB tak idealnie, aby jego skrót się zgadzał,
	podczas gdy inne niezsynchronizowane bloki w tym samym pasku zostaną
	zrekonstruowane z błędnymi danymi.

	Scenariusz ten nie zależy od kolizji skrótów; wymagałby raczej, aby same
	równania mieszanej parzystości przypadkowo odtworzyły dokładną,
	oryginalną zawartość 256-KiB bloku. Choć jest to matematycznie możliwe
	w przypadku specjalnie spreparowanych lub silnie powtarzalnych danych,
	jest to praktycznie niemożliwe w przypadku rzeczywistych danych.

	Jeśli wszystkie dyski są sprawne, dokończenie przerwanego `sync` przed
	wprowadzeniem dalszych zmian usuwa tę niejednoznaczność. Jeśli nastąpiła
	awaria dysku i konieczne jest odzyskanie danych, należy unikać modyfikacji
	dysków danych przed uruchomieniem `check` lub `fix`.

  Zastrzeżone nazwy plików i katalogów
	SnapRAID rezerwuje niektóre nazwy plików, katalogów i migawek do swoich
	wewnętrznych operacji. Nie używaj tych nazw dla danych użytkownika:

	.snapraidignore - Lokalny plik reguł wykluczeń.
	.snapraid - Katalog kontenera migawek.
	snapraid-* - Nazwy migawek ZFS zarządzanych przez SnapRAID.
	*.unrecoverable - Uszkodzone pliki objęte kwarantanną podczas odzyskiwania.
	CONTENT.tmp - Plik tymczasowy do aktualizacji pliku content.
	CONTENT.lock - Plik blokady pliku content.
	PARITY.spaceholder - Tymczasowy plik rezerwacji miejsca podczas alokacji
		parzystości w systemie Windows.

	Użycie tych nazw dla niepowiązanych danych może spowodować, że pliki lub
	migawki zostaną zignorowane, błędnie zinterpretowane jako metadane,
	nadpisane, przemianowane lub usunięte.

	W systemie Windows wszystkie warianty wielkości liter są również zastrzeżone,
	nawet w katalogach NTFS skonfigurowanych do rozróżniania wielkości liter
	(na przykład .SNAPRAIDIGNORE, .SnapRaid, CONTENT.TMP i CONTENT.Lock).
