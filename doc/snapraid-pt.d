...
Limitações (Limitations)
	O SnapRAID é um híbrido entre um RAID e um programa de backup, com o objetivo de combinar
	os melhores benefícios de ambos. No entanto, possui algumas limitações que deve
	considerar antes de o utilizar.

	A principal limitação é que, se um disco falhar, apenas poderá recuperar dados até ao
	estado da última operação de sincronização (`sync`). Quaisquer dados adicionados ou modificações feitas
	desde a última sincronização que estejam localizados no disco com falha serão perdidos.

	Consequentemente, o SnapRAID é adequado principalmente para dados que raramente mudam.

	Para dados que já existiam na última sincronização, a fiabilidade da recuperação depende
	...
	* Sem suporte a instantâneos, excluir ou alterar arquivos após um `sync`
		pode impedir a recuperação total de outros discos com falha. Isso ocorre
		porque a paridade já não corresponde aos arquivos modificados, mesmo que
		esses arquivos não estejam no disco com falha. Por outro lado, arquivos adicionados recentemente
		não impedem a recuperação de arquivos já existentes.
...
Primeiros Passos (Getting Started)
  ...
  Pooling (Pooling)
	...
	SnapRAID. Mergerfs oferece uma maneira mais flexível e eficiente de agrupar
  ...
  Recuperação (Recovering)
	...
	Por exemplo, se você removeu arquivos do array após o último
...
Comandos (Commands)
	...
	* Imprimir o estado do array -> `status`
	* Controlar os discos -> `smart`, `probe`, `up`, `down`
	* Criar um ponto de backup/recuperação -> `sync`
	* Verificar periodicamente os dados -> `scrub`
	* Restaurar o último ponto de backup/recuperação -> `fix`.
  ...
  diff
	...
	apresentado, agrupado por:
		...
		moved - Arquivos movidos ou renomeados no mesmo disco, mantendo
			o mesmo inode. Eles são identificados por terem o mesmo inode,
			tamanho e carimbo de data/hora, mas um caminho diferente. O nome
			do arquivo também pode ser diferente.
		copied - Arquivos copiados no mesmo disco ou em um disco
			...
			o nome.
		relocated - Arquivos movidos para um diretório ou disco diferente onde
			o arquivo original desapareceu e o inode não é mantido.
			Eles são identificados por terem o mesmo nome, tamanho e carimbo
			de data/hora. Se o carimbo de data/hora de subsegundos for zero, o
			caminho completo deve corresponder para ser identificado. Ao
			contrário dos arquivos 'moved', os arquivos relocados exigem que
	...
	Se um `sync` for necessário, o código de retorno do processo é 2,
  ...
  fix
	...
	Todos os arquivos e dados de paridade são comparados com o estado salvo no
	último `sync`. Se for encontrada uma diferença, esta é revertida para o estado armazenado.

	AVISO!
	O comando `fix` não diferencia erros de modificações intencionais.
	Reverte incondicionalmente o estado do arquivo para o do último `sync`.
	...
	arquivos que não foram alterados desde o último `sync`.
	...
	arquivos movidos desde a última operação de `sync`.
  ...
  locate
	Localiza arquivos armazenados nos discos de paridade. Para cada arquivo
	correspondente, imprime sua localização dentro do arquivo de paridade e o
	número de fragmentos que ocupa.

	Você pode usar a opção -t, --tail para restringir a operação aos arquivos
	que ocupam a parte final especificada da paridade.

	Se desejar realocar esses arquivos, você poderá usar a opção
...
Opções (Options)
	...
	-h, --pre-hash
		Em `sync`, executa uma fase de hash prexcluir de todos os novos dados
		para verificação adicional antes do cálculo da paridade.
		Normalmente, em `sync`, nenhum hash prexcluir é feito, e os novos
	...
	-t, --tail SIZE
		Limita a listagem de arquivos àqueles que não usam mais do que o
		tamanho final especificado dos discos de paridade.
		Você pode usar multiplicadores como K, M, G ou T (ex: --tail 1G).
		Esta opção só é válida quando usada em conjunto com o comando
		`locate`.
	...
	-W, --force-realloc-tail SIZE
		Funciona como -R, --force-realloc, mas limitado à parte final
		especificada (últimos SIZE bytes) de cada arquivo de paridade.
		Força a realocação (movimentação) de quaisquer fragmentos/blocos de arquivos
		atualmente armazenados nessa seção final, permitindo que sejam colocados
		em qualquer lugar no(s) arquivo(s) de paridade onde houver espaço livre
		disponível (incluindo espaços vazios existentes).
		O objetivo principal desta opção é diminuir o tamanho em disco
		do arquivo de paridade.
		Se a realocação limpar com sucesso toda a seção final (nenhum bloco
		permanecer usando-a), o arquivo de paridade será truncado,
		recuperando o espaço final não utilizado.
		Você pode usar multiplicadores como K, M, G ou T (ex:
		--force-realloc-tail 1G).
		Você pode usar locate -t, --tail para saber antecipadamente os
		arquivos afetados.
		ATENÇÃO! Esta opção é apenas para especialistas e é altamente
		recomendado não usá-la.
		Você NÃO tem proteção de dados durante a operação `sync` para os
		arquivos afetados.
	...
	-S, --start BLKSTART
		...
		e é suportada apenas por `check` e `fix`.
...
Configuração (Configuration)
  ...
  extra NAME DIR
	...
	Isso é útil para monitorar discos que não fazem parte do array,
  ...
  snapshot
	...
	para esse bloco, garantindo que ainda pode recuperar os dados com sucesso
	se outro disco falhar.

	Esta opção aplica-se exclusivamente a discos de dados formatados com os sistemas de
	arquivos Btrfs, Bcachefs ou ZFS em Linux e NTFS em Windows.
	Discos de paridade, ou discos de dados que utilizem outros sistemas de arquivos, usarão sempre
	a versão ativa do sistema de arquivos.

	A criação e exclusão de instantâneos requerem privilégios administrativos.
	Garanta que o SnapRAID é executado com as permissões necessárias (ex: sudo) quando
	esta opção estiver ativada.

	Consulte a seção SNAPSHOTS para uma explicação detalhada do ciclo de vida
	dos instantâneos.
  ...
  temp_limit TEMPERATURE_CELSIUS
	...
	depois que a temperatura do disco tiver subido quatro vezes, garantindo
  ...
  smartctl DISK/PARITY OPTIONS...
	...
	Você também pode especificar uma string personalizada de opções de informação
	usando a tag `[info: ...]` para substituir a opção padrão `-a`.
  ...
  smartignore DISK/PARITY ATTR [ATTR...]
	...
	Múltiplos atributos podem ser especificados separados por espaços.
	Cada atributo pode ser especificado como um número de 1 a 255, ou como um
	nome insensível a maiúsculas/minúsculas (por exemplo, `Current_Pending_Sector`).

	Por exemplo, para ignorar os atributos `Current Pending Sector Count` e
	`Reallocated Sectors Count` em todos os discos pelo seu número ou nome:

		:smartignore * 197 5
		:smartignore * Current_Pending_Sector Reallocated_Sector_Ct

	Para ignorá-los apenas no primeiro disco de paridade:
...
Instantâneos (Snapshots)
	...
	proteção adicional fornecida por instantâneos geridos inteiramente em
	segundo plano.

	Isso proporciona dois benefícios principais:
	...
	Recuperação - Se um arquivo for atualizado ou excluído do sistema de arquivos ativo,
		...
		no instantâneo para reconstruir o disco com falha.

		Em caso de falha de um disco durante um processo de sync ativo,
		o SnapRAID também é capaz de ler automaticamente tanto do instantâneo
		da computação de paridade anterior quanto da atual.
		Isso maximiza a probabilidade de uma recuperação completa ao
		fornecer acesso aos blocos de dados exatos necessários para resolver
		as equações de paridade, mesmo que esses blocos tenham sido modificados
		ou excluídos entre os syncs.

		Sem instantâneos, um arquivo atualizado ou excluído num disco saudável
		resulta na falta de blocos de dados que podem ser necessários para
		reparar outros discos com falha.

	Os instantâneos são criados apenas para discos de dados e apenas se o sistema de arquivos
	subjacente suportar esta funcionalidade. Os discos de paridade usam sempre
	o sistema de arquivos ativo.

	Atualmente, isso é suportado em Btrfs, Bcachefs e ZFS em Linux
	e em NTFS em Windows.

	Pode misturar discos de dados com sistemas de arquivos diferentes. Apenas aqueles que
	suportam instantâneos os utilizarão, enquanto os outros discos continuarão
	a operar diretamente no sistema de arquivos ativo.

	A criação e exclusão de instantâneos requerem privilégios administrativos.
	Garanta que o SnapRAID é executado com as permissões necessárias (ex: sudo)
  ...
  Comportamento dos Comandos com Instantâneos
	...
	Para os comandos `check` e `fix`, o uso do último instantâneo
	depende de discos específicos serem ou não alvo da operação utilizando a opção
	-d, --filter-disk.

	Um disco explicitamente selecionado via -d (o "alvo" da operação)
	usa sempre o sistema de arquivos ativo. Para `fix`, isso permite restaurar
	dados para a substituição do disco ativo. Para `check`, permite simular
	a operação `fix` sob as mesmas condições.

	Todos os outros discos de dados (os discos de "referência") serão acessados através dos
	seus instantâneos. Isso garante que, mesmo que esteja modificando arquivos
	nos seus discos saudáveis enquanto uma recuperação está em curso, o SnapRAID
	tem uma referência estável e congelada para resolver as equações de paridade.

	Se nenhuma opção -d for fornecida, o SnapRAID assume que a operação se aplica
	a todo o array, neste caso, `check` e `fix` usarão os sistemas
	de arquivos ativos exclusivamente.
...
Ciclo de Vida dos Instantâneos (Snapshots Lifecycle)
	...
	O instantâneo `stable` representa o estado do último
	`sync` concluído com sucesso, contendo os dados exatos utilizados para
	...
	Se um `sync` for interrompido, o instantâneo `pending` é preservado.
	Os comandos subsequentes `scrub`, `check` e `fix` utilizarão este
	instantâneo pendente, pois ele corresponde ao estado do sistema de arquivos registrado no
	arquivo .content, mesmo que a paridade esteja apenas parcialmente sincronizada.

	No caso de uma interrupção do `sync`, os comandos `check` e `fix`
	também leem do instantâneo `stable` para recuperar blocos de dados
	de arquivos que foram atualizados ou excluídos durante o sync.
	Isso é possível porque o arquivo .content retém metadados para
	todos os arquivos modificados ou excluídos ao longo do processo de sync.
	Esta informação histórica só é limpa quando um sync termina
	com sucesso e a versão final do arquivo .content é salva.
...
Padrão (Pattern)
	...
	colocá-los entre aspas para evitar que o shell os expanda.
...
Código de Saída (Exit Code)
	O SnapRAID termina com os seguintes códigos de erro:
...
Problemas Conhecidos (Known Issues)
  ...
  Correspondência de maiúsculas/minúsculas não-ASCII no Windows
	O SnapRAID utiliza codificação UTF-8 padrão e previsível em todas as plataformas,
	limitando a correspondência insensível a maiúsculas e minúsculas a caracteres
	ASCII.

	Os sistemas de arquivos do Windows sofrem com o design legado de forçar a
	insensibilidade a maiúsculas e minúsculas em nomes de arquivo Unicode por meio
	de tabelas de conversão arbitrárias e dependentes do sistema. Consequentemente,
	o Windows oculta a realidade do sistema de arquivos ao tratar sequências de bytes
	Unicode completamente diferentes como se fossem o mesmo arquivo.

	Por exemplo, `À.txt` e `à.txt` são nomes Unicode distintos, mas o Windows os
	trata ilogicamente como idênticos, enquanto o SnapRAID preserva sua verdadeira
	distinção.

	Essa limitação é causada pelo tratamento inconsistente de nomes de arquivo
	Unicode pelo Windows e por sua insistência no uso de aliases sem diferenciação de
	maiúsculas/minúsculas. Para evitar discrepâncias decorrentes da conversão do
	Windows, sempre especifique a combinação exata de maiúsculas e minúsculas para
	caracteres não-ASCII em caminhos de configuração e filtros.
  ...
  Recuperação após uma sincronização de multiparidade interrompida
	Em uma matriz com múltiplos discos de paridade, um `sync` interrompido
	pode deixar os níveis de paridade temporariamente fora de sincronia entre
	si. Por exemplo, a paridade 1 já pode conter dados recém-atualizados,
	enquanto a paridade 2 ainda contém os dados antigos.

	Durante `check` ou `fix`, o SnapRAID testa diferentes combinações de
	paridade e valida os dados reconstruídos em relação aos hashes de bloco
	conhecidos (cobrindo o bloco completo, normalmente 256 KiB).

	Se o SnapRAID tentar uma reconstrução usando uma combinação inconsistente
	de paridade antiga e nova, as equações lineares de RAID produzirão dados
	incorretos. Para que essa combinação inválida seja aceita por engano,
	as equações teriam que reconstruir acidentalmente pelo menos um bloco
	completo de 256 KiB de maneira tão perfeita que seu hash corresponda,
	enquanto outros blocos não sincronizados na mesma faixa são reconstruídos
	com dados incorretos.

	Esse cenário não depende de uma colisão de hash; em vez disso, as próprias
	equações de paridade mista teriam que gerar coincidentemente o conteúdo
	exato do bloco de 256 KiB original. Embora seja matematicamente concebível
	com dados especialmente preparados ou altamente repetitivos, é
	praticamente impossível com dados reais.

	Se todos os discos estiverem íntegros, concluir o `sync` interrompido
	antes de fazer novas alterações remove essa ambiguidade. Se um disco tiver
	falhado e a recuperação for necessária, evite modificar os discos de dados
	antes de executar `check` ou `fix`.

  Nomes reservados de arquivos e diretórios
	O SnapRAID reserva alguns nomes de arquivos, diretórios e snapshots para
	suas operações internas. Não use esses nomes para dados do usuário:

	.snapraidignore - Arquivo local de regras de exclusão.
	.snapraid - Diretório contêiner de snapshots.
	snapraid-* - Nomes de snapshots ZFS gerenciados pelo SnapRAID.
	*.unrecoverable - Arquivos danificados colocados em quarentena durante
		a recuperação.
	CONTENT.tmp - Arquivo temporário para atualizações do content.
	CONTENT.lock - Arquivo de bloqueio do content.
	PARITY.spaceholder - Arquivo temporário de reserva de espaço durante a
		alocação de paridade no Windows.

	O uso desses nomes para dados não relacionados pode fazer com que arquivos ou
	snapshots sejam ignorados, interpretados incorretamente como metadados,
	sobrescritos, renomeados ou excluídos.

	No Windows, todas as variações de maiúsculas e minúsculas também são reservadas,
	mesmo em diretórios NTFS configurados para diferenciar maiúsculas de minúsculas
	(por exemplo, .SNAPRAIDIGNORE, .SnapRaid, CONTENT.TMP e CONTENT.Lock).
...
Direitos Autorais (Copyright)
	Este arquivo é Copyright (C) 2026 Andrea Mazzoleni
