...
Limitaciones (Limitations)
	...
	* Con soporte para instantáneas (disponible en Btrfs, ZFS, Bcachefs y NTFS),
		SnapRAID puede mantener automáticamente una referencia congelada
		independiente de cada sistema de archivos de datos compatible.
...
Configuración (Configuration)
  ...
  snapshot
	...
	Cuando está habilitado, SnapRAID crea una instantánea de solo lectura de cada
	sistema de archivos de datos compatible al comienzo de un 'sync'. Esto proporciona una
	vista estable de cada sistema de archivos, evitando que modificaciones posteriores cambien
	los datos utilizados para el cómputo de paridad.
  ...
  smartctl DISK/PARITY OPTIONS...
	...
	También puede especificar una cadena de opciones de información
	personalizada mediante la etiqueta `[info: ...]` para sustituir
	la opción `-a` predeterminada.
  ...
  smartignore DISK/PARITY ATTR [ATTR...]
	...
	Se pueden especificar múltiples atributos separados por espacios.
	Cada atributo se puede especificar como un número del 1 al 255, o como un
	nombre que no distingue entre mayúsculas y minúsculas (p. ej., `Current_Pending_Sector`).

	Por ejemplo, para ignorar los atributos `Current Pending Sector Count` y
	`Reallocated Sectors Count` en todos los discos por su número o nombre:

		:smartignore * 197 5
		:smartignore * Current_Pending_Sector Reallocated_Sector_Ct

	Para ignorarlos solo en el primer disco de paridad:
...
Instantáneas (Snapshots)
	...
	utiliza instantáneas del sistema de archivos para mantener estables los datos leídos
	durante una operación, incluso cuando los archivos en los sistemas de archivos activos
	se modifican o eliminan posteriormente.
	...
	Consistencia - Los archivos modificados en el sistema de archivos activo durante un sync o scrub de
		...
		porque cada sistema de archivos con instantánea permanece sin cambios durante
		toda la operación.
  ...
  Comportamiento de comandos con instantáneas
	Los comandos `sync` y `scrub` utilizan instantáneas para los discos de datos cuyos
	sistemas de archivos las admiten. En un disco con instantánea, el comando ve
	una imagen estable del sistema de archivos no afectada por cambios concurrentes en
	el sistema de archivos activo.
	Durante un `sync`, se crea una nueva instantánea de forma independiente para cada
	sistema de archivos de datos compatible. Estas instantáneas proporcionan colectivamente
	los datos utilizados para el cómputo de paridad.
...
Ciclo de vida de las instantáneas (Snapshots Lifecycle)
	SnapRAID gestiona tres instantáneas específicas, `stable`, `pending` y `scan`,
	...
	La instantánea `scan` es una imagen temporal creada al inicio
	de un `sync` para proporcionar un estado congelado para el escaneo y el cómputo de paridad.
	Después de escanear y guardar el nuevo estado de contenido, reemplaza la instantánea
	`pending` anterior.

	La instantánea `pending` es la imagen utilizada por un `sync` interrumpido.
	...
	Sin embargo, si un `sync` se interrumpe, evite modificar, eliminar,
	reemplazar o mover archivos hasta que un `sync` posterior se complete
	con éxito. La paridad aún puede contener bloques calculados a partir de
	la instantánea utilizada por el sync interrumpido. Un sync posterior puede
	reemplazar esa instantánea, haciendo que las versiones intermedias de los
	archivos no estén disponibles para la recuperación.
...
Problemas conocidos (Known Issues)
  ...
  Múltiples configuraciones con instantáneas del sistema de archivos
	El soporte para instantáneas no se puede compartir de forma segura entre
	múltiples configuraciones independientes de SnapRAID que utilicen el mismo
	espacio de nombres de instantáneas del sistema de archivos.

	En Btrfs y Bcachefs, solo una configuración de SnapRAID con soporte de
	instantáneas habilitado puede usar directorios de datos dentro del mismo
	subvolumen. En ZFS, la misma restricción se aplica a un dataset, y en NTFS
	a un volumen.

	Las múltiples configuraciones que comparten el mismo espacio de nombres de
	instantáneas utilizan las mismas instantáneas `stable`, `pending` y `scan`.
	Por lo tanto, ejecutar una configuración puede reemplazar o eliminar las
	instantáneas de recuperación requeridas por otra.
  ...
  Recuperación con fix parcial
	...
	la finalización a nivel de archivo ni la cuarentena.

	Si la recuperación produce datos de mejor esfuerzo que no pueden demostrarse
	como representativos del contenido actual, dichos bloques aún pueden escribirse
	con el nombre de archivo normal. La incertidumbre asociada con estos bloques
	recuperados no se conserva una vez finalizado el comando.

	Como consecuencia, un `fix` posterior tratará estos bloques legibles como
	datos de archivo válidos y no intentará recuperarlos nuevamente. Esto puede
	hacer que una recuperación posterior informe éxito dejando datos incompletos
	o desactualizados en el archivo.

	Las opciones `-S` y `-B` están destinadas únicamente a la recuperación manual
  ...
  Recuperación tras una sincronización de multiparidad interrumpida
	En una matriz con múltiples discos de paridad, un `sync` interrumpido
	puede dejar los niveles de paridad temporalmente desincronizados entre sí.
	Por ejemplo, la paridad 1 podría contener datos recién actualizados,
	mientras que la paridad 2 todavía contiene los datos antiguos.

	Durante `check` o `fix`, SnapRAID prueba diferentes combinaciones de
	paridad y valida los datos reconstruidos con respecto a los hashes de
	bloque conocidos (que cubren el bloque completo, normalmente 256 KiB).

	Si SnapRAID intenta una reconstrucción utilizando una combinación
	inconsistente de paridad antigua y nueva, las ecuaciones lineales de RAID
	producirán datos incorrectos. Para que esta combinación no válida se
	acepte por error, las ecuaciones tendrían que reconstruir accidentalmente
	al menos un bloque completo de 256 KiB de manera tan perfecta que su hash
	coincida, mientras que otros bloques no sincronizados en la misma banda
	se reconstruyen con datos incorrectos.

	Este escenario no depende de una colisión de hashes; más bien, las propias
	ecuaciones de paridad mixta tendrían que generar de forma casual el
	contenido exacto del bloque de 256 KiB original. Si bien es matemáticamente
	concebible con datos especialmente preparados o muy repetitivos, es
	prácticamente imposible con datos reales.

	Si todos los discos están sanos, completar el `sync` interrumpido antes
	de realizar más cambios elimina esta ambigüedad. Si un disco ha fallado
