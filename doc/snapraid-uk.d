...
Опис (Description)
	...
	Насамперед призначений для домашніх медіацентрів з великими
	файлами, які рідко змінюються, SnapRAID пропонує кілька функцій:
...
Обмеження (Limitations)
	SnapRAID є гібридом між RAID і програмою резервного копіювання, що має на меті поєднати
	найкращі переваги обох рішень. Однак у нього є деякі обмеження, які вам слід
	врахувати перед використанням.

	Головне обмеження полягає в тому, що у разі збою диска ви можете відновити дані лише до
	стану останньої операції синхронізації (`sync`). Будь-які дані, додані або змінені
	з моменту останньої синхронізації, які розташовані на пошкодженому диску, будуть втрачені.

	Відповідно, SnapRAID насамперед підходить для даних, які рідко змінюються.

	Для даних, які вже існували на момент останньої синхронізації, надійність відновлення залежить
	від того, чи використовується опція знімка (`snapshot`):

	* За наявності підтримки знімків (доступно на Btrfs, ZFS, Bcachefs та NTFS),
		SnapRAID може автоматично підтримувати незалежну заморожену копію
		кожної підтримуваної файлової системи даних.
		Це гарантує, що навіть якщо ви зміните або видалите файли у вашій живій файловій системі
		під час або після синхронізації, процес відновлення залишиться узгодженим і надійним.

	* Без підтримки знімків видалення або зміна файлів після `sync`
		може запобігти повному відновленню інших дисків, що зазнали збою. Це відбувається
		тому, що парність більше не відповідає зміненим файлам, навіть якщо
		ці файли знаходяться не на пошкодженому диску. З іншого боку, щойно додані
		файли не перешкоджають відновленню вже існуючих файлів.
	...
	* Зберігаються лише імена файлів, часові мітки, символічні
		та жорсткі посилання. Права доступу, власники та
...
Початок роботи (Getting Started)
  ...
  Об'єднання (Pooling)
	...
	інструментом mergerfs, який тепер є рекомендованою опцією для
	користувачів Linux у спільноті SnapRAID. Mergerfs забезпечує
...
Команди (Commands)
	...
	* Створити точку резервного копіювання/відновлення -> `sync`
	* Періодично перевіряти дані -> `scrub`
	* Відновити останню точку резервного копіювання/відновлення -> `fix`.

	Команди обов'язково повинні бути написані малими літерами.
  ...
  smart
	...
	дозволяючи вам планувати своєчасну заміну дисків, які
	...
	помилку. У цьому випадку наполегливо рекомендується негайна
	заміна диска.
  ...
  diff
	...
	зведення змін, згруповане за:
		...
		moved (переміщені) - Файли, переміщені або перейменовані на тому ж
			диску зі збереженням того ж inode. Вони ідентифікуються за
			тим самим inode, розміром і міткою часу, але іншим шляхом.
			Назва файлу також може відрізнятися.
		copied (скопійовані) - Файли, скопійовані на той самий або інший диск, де
			оригінальний файл все ще існує.
			Вони ідентифікуються за однаковою назвою, розміром та міткою
			часу.
			Якщо мітка часу з точністю до часток секунди дорівнює нулю, для
			ідентифікації має збігатися повний шлях, а не лише назва.
		relocated (перерозміщені) - Файли, переміщені в інший каталог або на
			інший диск, де оригінальний файл зник, а inode не зберігся.
			Вони ідентифікуються за однаковою назвою, розміром та міткою часу.
			Якщо мітка часу з точністю до часток секунди дорівнює нулю, для
			ідентифікації має збігатися повний шлях. На відміну від файлів
			'moved', перерозміщені файли вимагають, щоб назва файлу залишалася
			незмінною.
		restored (відновлені) - Файли з іншим inode, але зі збігом каталогу, назви,
			розміру та мітки часу.
			Зазвичай це файли, відновлені після видалення.
  ...
  fix
	Відновлює всі файли та дані парності.

	Усі файли та дані парності порівнюються зі станом, збереженим під час
	останньої синхронізації `sync`. Якщо виявлено відмінність, стан повертається до збереженого.

	УВАГА!
	Команда `fix` не робить різниці між помилками та навмисними змінами.
	Вона безумовно повертає стан файлів до моменту останньої синхронізації `sync`.
  ...
  check
	...
	Ця команда насамперед призначена для ручної перевірки,
  ...
  touch
	Встановлює довільну часову мітку з точністю до часток секунди
  ...
  locate
	Знаходить файли, що зберігаються на дисках парності. Для кожного знайденого
	файлу виводиться його розташування всередині файлу парності та кількість
	фрагментів, які він займає.

	Ви можете використовувати параметр -t, --tail, щоб обмежити операцію файлами,
	що займають зазначену кінцеву частину (хвіст) парності.

	Якщо ви хочете перерозподілити ці файли, ви можете скористатися параметром
	-W, --force-realloc-tail.
	Майте на увазі, що такі файли не будуть захищені парністю під час процесу
	перерозподілу.
...
Опції (Options)
	...
	-f, --filter PATTERN
		...
		В Unix переконайтеся, що символи глобінгу взяті
	...
	-t, --tail SIZE
		Обмежує список файлів тими, що використовують не більше зазначеного
		розміру хвоста дисків парності.
		Можна використовувати множники, такі як K, M, G або T (наприклад, --tail 1G).
		Цей параметр дійсний лише при використанні разом із командою `locate`.
	...
	-R, --force-realloc
		...
		наполегливо не рекомендується її використовувати.
		Ви НЕ маєте захисту даних під час операції `sync`.

	-W, --force-realloc-tail SIZE
		Працює як -R, --force-realloc, але обмежується вказаною кінцевою
		частиною (останні SIZE-байт) кожного файлу парності.
		Примусово перерозподіляє (переміщує) будь-які фрагменти/блоки файлів,
		що наразі зберігаються в цій кінцевій секції, дозволяючи розмістити
		їх будь-де у файлі(ах) парності, де є вільне місце (включно з
		наявними пустотами).
		Основна мета цього параметра — зменшити розмір файлу парності на диску.
		Якщо перерозподіл успішно очистить всю кінцеву секцію (у ній не
		залишиться блоків), файл парності буде обрізаний, вивільняючи
		невикористаний простір хвоста.
		Можна використовувати множники, такі як K, M, G або T (наприклад,
		--force-realloc-tail 1G).
		Ви можете використовувати locate -t, --tail, щоб заздалегідь дізнатися
		про зачеплені файли.
		УВАГА!
		Цей параметр призначений лише для експертів, і його наполегливо
		не рекомендується використовувати.
		Під час операції `sync` захист даних для зачеплених файлів ВІДСУТНІЙ.
	...
	-S, --start BLKSTART
		...
		відновлення і підтримується лише `check` та `fix`.
...
Конфігурація (Configuration)
  ...
  z-parity FILE [,FILE] ...
	...
	Ця опція є альтернативою `3-parity`, насамперед
	призначена для малопотужних ЦП, таких як ARM або
  ...
  data NAME DIR
	...
	Однак налаштований шлях повинен постійно вказувати на ті самі
	базові дані. Якщо шлях містить символічні посилання,
	їхні цілі повинні залишатися стабільними. Перенаправлення
	таких посилань для зміни каталогу даних на льоту не підтримується.
  ...
  extra NAME DIR
	Визначає назву та точку монтування додаткових дисків для
	моніторингу за допомогою команд `smart` та `probe`.

	Це корисно для моніторингу дисків, які не є частиною масиву, але
	необхідні для функціонування системи, наприклад, завантажувального
	диска.

	Зверніть увагу, що на такі диски не впливають команди `up` та `down`,
	оскільки очікується, що вони завжди обертаються.

  snapshot
	Включає використання знімків файлової системи для команд `sync`, `scrub`,
	`check` та `fix`.

	Якщо увімкнено, SnapRAID створює знімок лише для читання кожної підтримуваної
	файлової системи даних на початку операції 'sync'. Це забезпечує стабільний зріз
	кожної файлової системи, запобігаючи зміні даних, що використовуються для обчислення
	парності, подальшими модифікаціями.

	Це значно покращує відновлення: якщо файл видаляється з живої файлової системи,
	він залишається збереженим у знімку. Це запобігає «порушенню» парності
	для цього блоку, гарантуючи, що ви все одно зможете успішно відновити дані,
	якщо інший диск зазнає збою.

	Ця опція застосовується виключно до дисків із даними, відформатованих у файлових
	системах Btrfs, Bcachefs або ZFS у Linux та NTFS у Windows.
	Диски парності або диски з даними, що використовують інші файлові системи, завжди використовуватимуть
	живу версію файлової системи.

	Створення та видалення знімків вимагають прав адміністратора.
	Переконайтеся, що SnapRAID запущено з необхідними привілеями (наприклад, sudo), коли
	увімкнено цю опцію.

	Детальне пояснення життєвого циклу знімків див. у розділі SNAPSHOTS.
  ...
  exclude/include PATTERN
	...
	Зверніть увагу, що фільтрація відбувається ієрархічно під час
	обходу каталогів. Якщо каталог відповідає правилу `exclude`, весь
	каталог пропускається (відсікається), і будь-які правила `include`
	для файлів або підкаталогів у ньому не оцінюватимуться.
  ...
  blocksize SIZE_IN_KIBIBYTES
	...
	УВАГА! Ця опція лише для експертів, і наполегливо
	...
	даних. Однак, втрачена парність не накопичується
	між дисками даних. Марно втрачений простір внаслідок великої
	...
	розміру блоку втрачається марно для кожного файлу.
	Наприклад, зі 100 000 файлами та розміром блоку 256 КіБ
	ви втратите 12,8 ГБ парності, що може призвести до
  ...
  hashsize SIZE_IN_BYTES
	...
	Інший розмір хешу варто використовувати, якщо ваша
	система має обмежену пам'ять.
	Зазвичай SnapRAID потребує 1 ГіБ ОЗП на кожні 16 ТБ
	даних у масиві.

	Зокрема, для зберігання хешів даних SnapRAID
	вимагає приблизно TS*(1+HS)/BS байтів
	...
	:RAM = (8 * 4 * 10^12) * (1+16) / (256 * 2^10) = 1.93 ГіБ

	Перехід до розміру хешу 8, ви отримуєте:

	:RAM = (8 * 4 * 10^12) * (1+8) / (256 * 2^10) = 1.02 ГіБ

	Перехід до розміру блоку 512, ви отримуєте:

	:RAM = (8 * 4 * 10^12) * (1+16) / (512 * 2^10) = 0.96 ГіБ

	Перехід до розміру хешу 8 та розміру блоку 512, ви отримуєте:

	:RAM = (8 * 4 * 10^12) * (1+8) / (512 * 2^10) = 0.51 ГіБ
  ...
  temp_limit TEMPERATURE_CELSIUS
	...
	Для виконання цього аналізу SnapRAID потрібне
	опорне значення температури системи.
	...
	запуску, як резервне опорне значення.
  ...
  smartignore DISK/PARITY ATTR [ATTR...]
	...
	Можна вказати кілька атрибутів, розділених пробілами.
	Кожен атрибут може бути вказаний як число від 1 до 255 або як регістронезалежне
	ім'я (наприклад, `Current_Pending_Sector`).

	Наприклад, щоб ігнорувати атрибути `Current Pending Sector Count` та
	`Reallocated Sectors Count` на всіх дисках за їхнім номером або ім'ям:

		:smartignore * 197 5
		:smartignore * Current_Pending_Sector Reallocated_Sector_Ct

	Щоб ігнорувати їх лише на першому диску парності:
...
Знімки (Snapshots)
	Якщо в конфігурації увімкнено опцію знімків, SnapRAID використовує знімки
	файлової системи для збереження стабільності даних, що зчитуються під час
	операції, навіть якщо файли в живих файлових системах згодом змінюються
	або видаляються.

	Керування знімками відбувається повністю автоматично та прозоро.
	Ви можете продовжувати використовувати SnapRAID точно так само, як і раніше, отримуючи
	додатковий захист, що забезпечується знімками, які повністю обробляються у
	фоновому режимі.

	Це дає дві основні переваги:

	Узгодженість — Файли, змінені в живій файловій системі під час тривалого
		процесу sync або scrub, не викликатимуть невідповідності парності або переривання
		операцій, оскільки кожна файлова система зі знімком залишається незмінною
		протягом усієї операції.
	Відновлення — Якщо файл оновлюється або видаляється з живої файлової системи,
		він залишається збереженим у знімку. Якщо збій диска станеться
		до запуску наступної синхронізації sync, SnapRAID використає дані, збережені
		в знімку, для реконструкції диска, що зазнав збою.

		У разі збою диска під час активного процесу sync,
		SnapRAID також може автоматично читати як зі знімка попереднього
		обчислення парності, так і з поточного.
		Це максимізує ймовірність повного відновлення, надаючи
		доступ до точних блоків даних, необхідних для вирішення рівнянь парності,
		навіть якщо ці блоки були змінені або видалені між синхронізаціями.

		Без знімків оновлений або видалений файл на справному диску
		призводить до відсутності блоків даних, які можуть знадобитися для
		відновлення інших дисків, що зазнали збою.

	Знімки створюються лише для дисків із даними і лише в тому випадку, якщо базова
	файлова система підтримує цю функціональність. Диски парності завжди використовують
	живу файлову систему.

	Наразі це підтримується на Btrfs, Bcachefs та ZFS в Linux
	і на NTFS у Windows.

	Ви можете комбінувати диски з даними з різними файловими системами. Тільки ті з них,
	які підтримують знімки, використовуватимуть їх, тоді як інші диски продовжуватимуть
	працювати безпосередньо з живою файловою системою.

	Створення та видалення знімків вимагають прав адміністратора.
	Переконайтеся, що SnapRAID запущено з необхідними привілеями (наприклад, sudo),
	коли знімки увімкнено.

  Поведінка команд із знімками
	Команди `sync` та `scrub` використовують знімки для дисків із даними, чиї
	файлові системи їх підтримують. На диску зі знімком команда бачить стабільний
	образ файлової системи, не зачеплений одночасними змінами в живій файловій
	системі.
	Під час `sync` новий знімок створюється незалежно для кожної підтримуваної
	файлової системи даних. Ці знімки сукупно надають дані, що використовуються
	для обчислення парності.
	Навпаки, команда `scrub` використовує останній знімок, створений під час
	найостаннішої синхронізації, щоб підтримувати надійну точку відліку,
	яка відповідає існуючій парності.

	Для команд `check` та `fix` використання останнього знімка
	залежить від того, чи націлені конкретні диски за допомогою опції
	-d, --filter-disk.

	Диск, явно вибраний за допомогою -d («ціль» операції),
	завжди використовує живу файлову систему. Для `fix` це дозволяє відновити
	дані на активну заміну диска. Для `check` це дозволяє симулювати
	операцію `fix` за тих самих умов.

	Доступ до всіх інших дисків із даними («довідкових» дисків) здійснюватиметься через
	їхні знімки. Це гарантує, що навіть якщо ви змінюєте файли
	на своїх справних дисках під час відновлення, SnapRAID
	має стабільну, замороженну точку відліку для вирішення рівнянь парності.

	Якщо опція -d не надана, SnapRAID припускає, що операція застосовується
	до всього масиву; у цьому випадку `check` та `fix` використовуватимуть виключно
	живі файлові системи.

	Усі інші команди працюють виключно з живою файловою системою.

Життєвий цикл знімків (Snapshots Lifecycle)
	SnapRAID керує трьома конкретними знімками, `stable`, `pending` та `scan`,
	всередині прихованого каталогу в корінному каталозі кожного підтому даних.
	У Btrfs, Bcachefs та NTFS використовується каталог `.snapraid/`,
	у ZFS — стандартний `.zfs/snapshot/`.

	Знімок `stable` представляє стан останньої успішно
	завершеної синхронізації `sync`, що містить точні дані, використані для
	обчислення поточної парності. Він служить основним джерелом даних
	для команд `scrub`, `check` та `fix`.

	Знімок `scan` — це тимчасовий образ, що створюється на початку
	`sync` для забезпечення замороженого стану для сканування та обчислення парності.
	Після сканування та збереження нового стану вмісту він замінює попередній
	знімок `pending`.

	Знімок `pending` — це образ, використовуваний перерваною синхронізацією `sync`.
	Після успішного завершення операції попередній знімок `stable`
	видаляється, а знімок `pending` підвищується в статусі, щоб
	зайняти його місце в якості нового стабільного орієнтира.

	Якщо процес `sync` переривається, знімок `pending` зберігається.
	Наступні команди `scrub`, `check` та `fix` використовуватимуть цей
	очікуваний знімок, оскільки він відповідає стану файлової системи, записаному в
	файлі .content, навіть якщо парність синхронізована лише частково.

	У разі переривання `sync` команди `check` та `fix`
	також читають дані зі знімка `stable` для вилучення блоків даних
	із файлів, які були оновлені або видалені під час синхронізації.
	Це можливо тому, що файл .content зберігає метадані для
	усіх змінених або видалених файлів протягом усього процесу sync.
	Ця історична інформація очищається лише після успішного завершення sync
	і збереження фінальної версії файлу .content.

	Якщо `sync` перезапускається після переривання, існуючий
	очікуваний (pending) знімок видаляється, і створюється новий для
	фіксації поточного стану живої файлової системи.
	Однак, якщо `sync` було перервано, уникайте зміни, видалення,
	заміни або переміщення файлів, доки наступний `sync` не завершиться
	успішно. Парність все ще може містити блоки, обчислені на основі знімка,
	використаного перерваною синхронізацією. Пізніша синхронізація може
	замінити цей знімок, зробивши проміжні версії файлів недоступними для
	відновлення.
...
Шаблон (Pattern)
	...
	Символ підстановки подвійна зірочка `**` зіставляється через роздільники
	каталогів лише тоді, коли він прилягає до роздільника каталогів (перед ним або
	після нього стоїть скісна риска). У цій формі він відповідає нулю або більше
	символам на кількох рівнях каталогів. Наприклад, `foo/**/bar` відповідає
	`foo/bar`, `foo/a/bar` та `foo/a/b/bar`; `**/build` відповідає `build` та
	`foo/bar/build`; а `src/**.js` відповідає `src/main.js` та
	`src/ui/components/main.js`.
	Окрема форма `/**/` не приймається як повний шаблон.

	Коли `**` не прилягає до скісної риски, він поводиться як `*` і не відповідає
	роздільникам каталогів. Наприклад, `**.jpg` відповідає `photo.jpg` у поточному
	каталозі, але не відповідає `dir/photo.jpg`. Для цієї мети використовуйте
	`**/*.jpg`.
	...
	шляху, тоді як `**` може відповідати через роздільники каталогів лише тоді,
	коли прилягає до скісної риски.

	Існує чотири різні типи шаблонів:

	=FILE
		Вибирає будь-який файл з ім'ям FILE.
		Цей шаблон застосовується тільки до файлів, а не до каталогів.

	=DIR/
		Вибирає будь-який каталог з ім'ям DIR і все, що знаходиться всередині.
		Цей шаблон застосовується тільки до каталогів, а не до файлів.

	=/PATH/FILE
		Вибирає точно вказаний шлях до файлу. Цей шаблон застосовується
		тільки до файлів, а не до каталогів.

	=/PATH/DIR/
		Вибирає точно вказаний шлях до каталогу і все, що знаходиться
		всередині. Цей шаблон застосовується тільки до каталогів, а не до файлів.
	...
	їх інтерпретації як символу глобінгу.
	...
	Зверніть увагу, що виключення каталогу (за допомогою DIR/ або /ШЛЯХ/DIR/)
	запобігає входу SnapRAID у цей каталог та його скануванню. Відповідно,
	неможливо повторно включити файл або підкаталог, розташований усередині
	виключеного батьківського каталогу, оскільки сканер ніколи не переходить у нього.

	Наприклад, наступна конфігурація НЕ включить `file.dat`, оскільки
	`/keep/` відсікається до того, як `file.dat` буде перевірено:

		:# НЕ працює як очікується: /keep/ повністю пропускається
		:include /keep/file.dat
		:exclude /keep/

	Щоб виключити файли всередині каталогу, зберігши конкретні файли,
	виключайте шаблон файлів усередині каталогу замість виключення самого каталогу:

		:# Працює: /keep/ сканується, але інші файли всередині виключаються
		:include /keep/file.dat
		:exclude /keep/*
	...
	Наприклад:
		...
		:# розширення глобінгу оболонкою.
		:snapraid -f "*.mp3" check

	В Unix, при використанні символів глобінгу в
...
Вміст (Content)
	...
	Файл вмісту зчитується та записується командами `sync`, `scrub`,
	`touch` та `rehash`. Він зчитується командами `status`, `diff`,
	`check`, `fix`, `list`, `dup`, `locate` та `pool`, і повністю
	ігнорується командами `smart`, `probe`, `up`, `down` та `devices`.
...
Код завершення (Exit Code)
	SnapRAID завершує роботу з такими кодами помилок:

	0 - Все в порядку.
	1 - Команда виявила помилки.
	2 - Команда `diff` визначила, що все в порядку, але потрібен `sync`.
	3 - Команда дискового пристрою (`probe`, `smart`, `devices`, `up`,
		`down`) завершена, але принаймні одна файлова система працює
		у деградованому режимі з відсутніми пристроями.

Відомі проблеми (Known Issues)
  Межі файлової системи
	SnapRAID не перетинає межі файлових систем, точок монтування або підтомів
	під час сканування диска даних. Коли виявляється така межа, SnapRAID
	повідомляє ПОПЕРЕДЖЕННЯ і не виконує сканування за її межами.

	Файли, розташовані за такими межами, не включаються до диска даних і
	повинні бути налаштовані окремо, якщо їх потрібно захистити.

  Точки монтування зі знімками файлової системи
	Коли знімки файлової системи увімкнено, вкладені точки монтування та
	bind-монтування всередині диска даних не підтримуються.

	Точки монтування є частиною простору імен монтування працюючої системи і
	не зберігаються у знімках файлової системи. Отже, під час сканування знімка
	каталог, прихований точкою монтування в активній файловій системі, може
	стати видимим.

	Не розміщуйте вкладені точки монтування або bind-монтування всередині дисків
	даних, коли ввімкнено знімки файлової системи.

  Кілька конфігурацій зі знімками файлової системи
	Підтримка знімків не може безпечно спільно використовуватися кількома
	незалежними конфігураціями SnapRAID, які використовують один і той самий
	простір імен знімків файлової системи.

	У Btrfs та Bcachefs лише одна конфігурація SnapRAID з увімкненою підтримкою
	знімків може використовувати каталоги даних у межах одного підтому. У ZFS
	це ж обмеження стосується датасету, а в NTFS — тому.

	Кілька конфігурацій, які спільно використовують один і той самий простір
	імен знімків, використовують однакові знімки `stable`, `pending` та `scan`.
	Запуск однієї конфігурації може замінити або видалити знімки відновлення,
	необхідні для іншої.

  Модифікації файлів на місці без зміни позначок часу
	SnapRAID виявляє модифікації файлів шляхом порівняння розмірів файлів та
	позначок часу модифікації (mtime).

	Якщо програма змінює файл на місці без зміни його розміру або оновлення
	позначки часу модифікації, "diff" та "sync" не виявлять, що файл змінився,
	і його парність не буде оновлено.

	Щоб гарантувати, що SnapRAID розпізнає такі зміни, оновіть позначку часу
	модифікації файлу (наприклад, за допомогою утиліти "touch"). В іншому
	випадку різниця буде виявлена лише як невідповідність хешу під час
	наступного "scrub" або "check".

  Зіставлення регістру символів non-ASCII у Windows
	SnapRAID використовує стандартне, передбачуване кодування UTF-8 на всіх
	платформах, обмежуючи нечутливе до регістру зіставлення символами ASCII.

	Файлові системи Windows страждають від застарілого архітектурного рішення,
	що нав'язує нечутливість до регістру для імен файлів Unicode за допомогою
	довільних, залежних від системи таблиць перетворення. Як наслідок, Windows
	приховує реальність файлової системи, обробляючи абсолютно різні
	послідовності байтів Unicode так, ніби це один і той самий файл.

	Наприклад, `À.txt` та `à.txt` є різними іменами Unicode, але Windows нелогічно
	вважає їх ідентичними, тоді як SnapRAID зберігає їхню справжню відмінність.

	Це обмеження спричинене непослідовною обробкою імен файлів Unicode у Windows
	та її наполегливим використанням псевдонімів без урахування регістру. Щоб
	запобігти розбіжностям через зведення регістру у Windows, завжди вказуйте
	точний регістр для будь-яких символів non-ASCII у шляхах конфігурації та
	фільтрах.

  Суфікс невідновлюваних файлів
	Коли `fix` не може повністю відновити пошкоджений файл, він поміщає його в
	карантин, додаючи розширення `.unrecoverable` до його імені.

	Якщо наступний `fix` повторює спробу відновлення, він створює новий файл зі
	звичайним ім'ям і зберігає старий файл `.unrecoverable` під час виконання
	відновлення. Якщо повторна спроба зазнає невдачі, новий невдалий результат
	замінює старий карантин. Якщо повторна спроба повністю успішна, SnapRAID
	видаляє застарілий файл `.unrecoverable`. Файл карантину ніколи не повертається
	до звичайного імені.

	Оскільки SnapRAID ідентифікує файли карантину виключно за назвою файлу, будь-який
	раніше існуючий файл, який випадково використовує суфікс `.unrecoverable`, може
	бути усічений, перезаписаний, замінений або видалений, якщо його базова назва
	збігається з файлом, що відновлюється.

	Уникайте використання розширення `.unrecoverable` для звичайних файлів і
	переконайтеся, що правило `exclude *.unrecoverable` включено до вашої
	конфігурації, щоб такі файли не відстежувалися в наборі даних парності.

  Відновлення з частковим fix
	Коли `fix` обмежено за допомогою `-S, --start` або `-B, --count`,
	SnapRAID працює лише з вибраними блоками і не виконує фіналізацію
	на рівні файлів або переміщення в карантин.

	Якщо в результаті відновлення отримано дані найкращої спроби (best-effort),
	щодо яких не можна довести відповідність поточному вмісту, такі блоки все
	одно можуть бути записані під звичайним ім'ям файлу. Невизначеність, пов'язана
	з цими відновленими блоками, не зберігається після завершення команди.

	Як наслідок, наступний `fix` сприйматиме ці доступні для читання блоки як дійсні
	дані файлу і не намагатиметься відновити їх знову. Це може призвести до того,
	що подальше відновлення повідомить про успіх, залишивши у файлі неповні або
	застарілі дані.

	Параметри `-S` та `-B` призначені лише для розширеного ручного відновлення,
	зазвичай після збою звичайного `fix`. Якщо частковий `fix` повідомляє про
	невідновлювані дані, видаліть або перейменуйте уражені файли перед спробою
	повторного відновлення або перевірте їхній вміст вручну.

  Жорсткі посилання NTFS та виключені шляхи
	У файловій системі NTFS записи каталогів кешують розмір файлу та часові
	позначки незалежно для кожного жорсткого посилання. Коли файл змінюється
	через одне посилання, записи каталогів інших посилань, що вказують на
	той самий файл, можуть зберігати застарілі метадані, доки до них не
	буде здійснено доступ.

	SnapRAID автоматично виявляє та нормалізує цей стан щоразу, коли під
	час сканування виявляється щонайменше два жорстких посилання на один
	і той самий файл. Однак, якщо сканується лише одне жорстке посилання,
	тоді як інші виключені фільтрами або знаходяться за межами шляху диска
	даних, SnapRAID не може виявити колізію inode і покладається на
	кешовані метадані єдиного видимого посилання.

	Отже, зміни, внесені через виключене або зовнішнє жорстке посилання,
	можуть залишитися непоміченими для SnapRAID, доки метадані включеного
	посилання не будуть оновлені операційною системою (наприклад, коли до
	файлу звертаються або відкривають його за цим шляхом).

	Щоб уникнути цього обмеження, не змінюйте захищені файли через жорсткі
	посилання, виключені зі SnapRAID, або переконайтеся, що всі жорсткі
	посилання на файл включені до сканування, щоб SnapRAID міг їх нормалізувати.

  Відновлення після перерваного sync з кількома парностями
	У масиві з кількома дисками парності перерваний `sync` може залишити
	рівні парності тимчасово розсинхронізованими між собою. Наприклад,
	парність 1 вже може містити оновлені дані, тоді як парність 2 все ще
	містить старі дані.

	Під час виконання `check` або `fix` SnapRAID випробовує різні комбінації
	парностей і перевіряє відновлені дані за відомими хешами блоків
	(що охоплюють блок цілком, зазвичай 256 КіБ).

	Якщо SnapRAID намагається виконати відновлення, використовуючи
	неузгоджену суміш старої та нової парності, лінійні рівняння RAID
	дадуть неправильні дані. Щоб така недійсна комбінація була помилково
	прийнята, рівняння мали б випадково відновити хоча б один повний блок
	розміром 256 КіБ настільки ідеально, щоб його хеш збігся, тоді як інші
	несинхронізовані блоки в тій самій смузі будуть відновлені з помилковими
	даними.

	Цей сценарій не залежить від колізії хешів; навпаки, самі рівняння
	змішаної парності мали б випадково відтворити точний вихідний вміст
	256-КіБ блоку. Хоча це математично можливо для спеціально підготовлених
	або сильно повторюваних даних, на реальних даних це практично неможливо.

	Якщо всі диски справні, завершення перерваного `sync` перед внесенням
	подальших змін усуває цю неоднозначність. Якщо диск вийшов з ладу і
	потрібне відновлення, уникайте змін на дисках даних перед запуском
	`check` або `fix`.

Переклад (Translation)
	Цей документ є автоматичним перекладом англійського посібника.
	Зверніться до англійського посібника для отримання офіційної версії.

Авторське право (Copyright)
	Цей файл захищений авторським правом (C) 2026 Andrea Mazzoleni
