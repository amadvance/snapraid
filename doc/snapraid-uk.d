...
Відомі проблеми (Known Issues)
  ...
  Жорсткі посилання (Hard links)
	SnapRAID визначає одне жорстке посилання як оригінальний файл і представляє
	інші жорсткі посилання як посилання на нього. Яке саме посилання вибирається
	як оригінальний файл, залежить від порядку сканування записів каталогу.

	Якщо цей порядок змінюється, наприклад, після перейменування, відновлення
	файлів або повторного створення каталогів, команда "diff" може повідомляти
	про уявні переміщення, оновлення, додавання або видалення, навіть коли
	пов'язані файли все ще представляють ті самі дані.

	Це не свідчить про пошкодження даних, але повідомлені відмінності можуть
	викликати плутанину і можуть не відповідати безпосередньо операціям,
	виконаним у файловій системі.
