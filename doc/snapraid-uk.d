...
Обмеження (Limitations)
	...
	* За наявності підтримки знімків (доступно на Btrfs, ZFS, Bcachefs та NTFS),
		SnapRAID може автоматично підтримувати заморожену копію ваших дисків із даними.
...
Конфігурація (Configuration)
  ...
  snapshot
	...
	Якщо увімкнено, SnapRAID створює знімок ваших даних лише для читання на
	початку операції 'sync'. Це забезпечує узгоджений зріз ваших файлів на певний момент часу,
	запобігаючи помилкам, викликаним одночасним зміненням файлів.
...
Знімки (Snapshots)
	Якщо в конфігурації увімкнено опцію знімків, SnapRAID використовує
	функціональність знімків файлової системи для забезпечення атомарності та
	узгодженості операцій.
	...
	Узгодженість — Файли, змінені в живій файловій системі під час тривалого
		...
		операцій, оскільки SnapRAID бачить заморожений знімок на момент часу.
  ...
  Поведінка команд із знімками
	Команди `sync` та `scrub` працюють виключно зі знімками,
	щоб гарантувати, що всі операції з даними виконуються на узгодженому,
	замороженому стані. Використовуючи ці знімки, SnapRAID запобігає невідповідності
	парності, яка в іншому випадку була б викликана одночасними змінами файлів у живій файловій системі.
	Під час `sync` команда використовує новий знімок, створений на початку
	процесу, для фіксації поточного стану для обчислення парності.
...
Життєвий цикл знімків (Snapshots Lifecycle)
	SnapRAID керує двома конкретними знімками, `stable` та `pending`,
	...
	Знімок `pending` — це тимчасовий образ, що створюється на початку
	`sync` для забезпечення замороженого стану для обчислення парності.
