...
Ограничения (Limitations)
	...
	* При наличии поддержки снимков (доступно на Btrfs, ZFS, Bcachefs и NTFS),
		SnapRAID может автоматически поддерживать замороженную копию ваших дисков с данными.
...
Конфигурация (Configuration)
  ...
  snapshot
	...
	Если опция включена, SnapRAID создает снимок ваших данных только для чтения в
	начале операции 'sync'. Это обеспечивает согласованный срез ваших файлов на определенный
	момент времени, предотвращая ошибки, вызванные одновременным изменением файлов.
...
Снимки (Snapshots)
	Если в конфигурации включена опция снимков, SnapRAID использует
	функциональность снимков файловой системы для обеспечения атомарности и
	...
	Согласованность — Файлы, измененные в живой файловой системе во время длительного
		...
		операций, поскольку SnapRAID видит замороженный снимок на момент времени.
  ...
  Поведение команд при использовании снимков
	Команды `sync` и `scrub` работают исключительно со снимками,
	чтобы гарантировать, что все операции с данными выполняются на согласованном,
	замороженном состоянии. Используя эти снимки, SnapRAID предотвращает несоответствия
	четности, которые в противном случае были бы вызваны одновременными изменениями файлов в живой файловой системе.
	Во время `sync` команда использует новый снимок, созданный в начале
	процесса, для фиксации текущего состояния для вычисления четности.
...
Жизненный цикл снимков (Snapshots Lifecycle)
	SnapRAID управляет двумя конкретными снимками, `stable` и `pending`,
	...
	Снимок `pending` — это временный образ, создаваемый в начале
	`sync` для обеспечения замороженного состояния для вычисления четности.
