...
Ограничения (Limitations)
	...
	с момента последней синхронизации и находящиеся на давшем сбой диске, будут потеряны.
	...
	* Без поддержки снимков удаление или изменение файлов после `sync`
		может предотвратить полное восстановление других дисков, давших сбой. Это происходит
		потому, что четность больше не соответствует измененным файлам, даже если
		эти файлы находятся не на давшем сбой диске. С другой стороны, недавно добавленные
...
Параметры (Options)
	...
	-W, --force-realloc-tail РАЗМЕР
		Работает как -R, --force-realloc, но ограничивается указанной хвостовой
		частью (последние РАЗМЕР байт) каждого файла четности.
...
Снимки (Snapshots)
	...
	Восстановление — Если файл обновляется или удаляется из живой файловой системы,
		...
		в снимке, для реконструкции давшего сбой диска.
		...
		восстановления других дисков, давших сбой.
