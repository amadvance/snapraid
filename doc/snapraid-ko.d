...
제한 사항 (Limitations)
	...
	* 스냅샷 지원이 있는 경우 (Btrfs, ZFS, Bcachefs 및 NTFS에서 사용 가능):
		SnapRAID는 데이터 디스크의 동결된 참조를 자동으로 유지할 수 있습니다.
...
설정 (Configuration)
  ...
  snapshot
	...
	활성화되면 SnapRAID는 `sync` 시작 시 데이터의 읽기 전용 스냅샷을 생성합니다.
	이는 파일의 일관된 특정 시점 뷰를 보장하여 동시 파일 수정으로 인해 발생하는
	오류를 방지합니다.
...
스냅샷 (Snapshots)
	...
	일관성 - 장시간 실행되는 sync 또는 scrub 중에 라이브 파일시스템에서 수정된 파일은
		SnapRAID가 동결된 시점의 이미지를 보기 때문에 패리티 불일치나 작업 중단을
		유발하지 않습니다.
  ...
  스냅샷 사용 시 명령어 동작 (Command Behavior with Snapshots)
	`sync` 와 `scrub` 명령어는 모두 일관되고 동결된 상태에 대해 모든 데이터 작업이 수행되도록
	보장하기 위해 스냅샷에서만 독점적으로 작동합니다. 이러한 스냅샷을 이용함으로써 SnapRAID는
	라이브 파일시스템의 동시 파일 수정으로 인해 발생할 수 있는 패리티 불일치를 방지합니다.
	`sync` 중에 명령어는 프로세스 시작 시 생성된 새 스냅샷을 사용하여 패리티 계산을 위한
	현재 상태를 캡처합니다.
...
스냅샷 수명 주기 (Snapshots Lifecycle)
	...
	`pending` 스냅샷은 패리티 계산을 위한 동결된 상태를 제공하기 위해 `sync` 시작 시
	생성되는 임시 이미지입니다.
