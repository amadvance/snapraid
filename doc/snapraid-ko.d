...
알려진 문제 (Known Issues)
  ...
  하드 링크
	SnapRAID는 하나의 하드 링크를 원본 파일로 식별하고 다른 하드 링크를 이에 대한
	참조로 표시합니다. 어떤 링크가 원본 파일로 선택되는지는 디렉터리 항목이 스캔되는
	순서에 따라 달라집니다.

	파일 이름 변경, 복원 또는 디렉터리 재생성 등으로 인해 이 순서가 변경되면,
	하드 링크된 파일이 여전히 동일한 데이터를 나타내더라도 "diff"에서 겉보기 이동,
	업데이트, 추가 또는 제거를 보고할 수 있습니다.

	이는 데이터 손상을 의미하지 않지만, 보고된 차이점이 혼란을 줄 수 있으며 수행된
	파일 시스템 작업과 직접 일치하지 않을 수 있습니다.
