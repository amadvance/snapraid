...
설명 (Description)
	...
	* 데이터 무결성을 보장하고 조용한 데이터 손상을 방지하기 위해 모든 데이터가
		해시됩니다.
...
제한 사항 (Limitations)
	...
	가장 큰 제한 사항은 디스크가 고장 날 경우, 마지막 `sync`(동기화) 작업의 상태까지만
	데이터를 복구할 수 있다는 점입니다. 마지막 동기화 이후에 추가되거나 수정된 데이터 중
	고장 난 디스크에 위치한 데이터는 모두 손실됩니다.

	따라서 SnapRAID는 주로 데이터 변경이 거의 없는 환경에 적합합니다.

	마지막 동기화 시점에 이미 존재했던 데이터의 경우, 복구 신뢰성은 `snapshot`(스냅샷)
	옵션의 사용 여부에 따라 달라집니다.

	* 스냅샷 지원이 있는 경우 (Btrfs, ZFS, Bcachefs 및 NTFS에서 사용 가능):
		SnapRAID는 지원되는 각 데이터 파일시스템의 독립적인 동결된 참조를
		자동으로 유지할 수 있습니다.
		이를 통해 동기화 중이나 동기화 후에 라이브 파일시스템의 파일을 수정하거나 삭제하더라도
		복구 프로세스가 일관되고 신뢰할 수 있게 유지됩니다.

	* 스냅샷 지원이 없는 경우:
		`sync` 후에 파일을 삭제하거나 변경하면 다른 고장 난 디스크의 완전한 복구가 방해받을 수
		있습니다. 이는 수정된 파일이 고장 난 디스크에 없더라도 패리티가 수정된 파일과
		더 이상 일치하지 않기 때문에 발생합니다. 반면에 새로 추가된 파일은 이미 존재하는
		파일의 복구를 방해하지 않습니다.
...
시작하기 (Getting Started)
  ...
  스크러빙 (Scrubbing)
	...
	이 프로세스 중에 조용한 오류 또는 입출력 오류가 발견되면, 해당 블록은
...
명령어 (Commands)
	...
	* 마지막 백업/복구 지점 복원 -> `fix`
	...
	명령어는 반드시 소문자로 작성해야 합니다.

  status
	디스크 어레이의 상태 요약을 출력합니다.

	패리티 단편화, 확인 없이 블록이 얼마나 오래되었는지, 그리고
	스크럽 중에 발생한 모든 기록된 조용한 오류에 대한 정보를 포함합니다.
  ...
  diff
	...
	제시됩니다:
		...
		moved - 동일한 inode를 유지하면서 동일한 디스크에서 이동되거나
			이름이 변경된 파일. 동일한 inode, 크기 및 타임스탬프를 가지지만
			경로가 다른 것으로 식별됩니다. 파일 이름도 다를 수 있습니다.
		copied - 원본 파일이 여전히 존재하는 동일한 디스크 또는 다른
			디스크에 복사된 파일.
			이들은 동일한 이름, 크기 및 타임스탬프를 가짐으로써 식별됩니
			다.
			하위 초 타임스탬프가 0인 경우, 이름뿐만 아니라 전체 경로가
			일치해야 식별됩니다.
		relocated - 원본 파일이 사라지고 inode가 유지되지 않는, 다른
			디렉토리 또는 디스크로 이동된 파일.
			이들은 동일한 이름, 크기 및 타임스탬프를 가짐으로써 식별됩니다.
			하위 초 타임스탬프가 0인 경우, 식별을 위해 전체 경로가 일치해야
			합니다. 'moved' 파일과 달리, 재배치된 파일은 파일 이름이 동일해야
			합니다.
		restored - inode는 다르지만 디렉토리, 이름, 크기 및 타임스탬
			프가 일치하는 파일.
			이들은 대개 삭제된 후 복구된 파일들입니다.
  ...
  sync
	...
	이 프로세스 중에 조용한 오류 또는 입출력 오류가 발견되면,
	해당 블록은 bad로 표시됩니다.
  ...
  scrub
	어레이를 스크럽하여 데이터 및 패리티 디스크의 조용한 오류 또는 입출력 오류를
	확인합니다.
	...
	발견된 모든 조용한 오류 또는 입출력 오류에 대해, 해당 블록은 `content` 파일에
	bad로 표시됩니다.
	...
	어레이에서만 `scrub`을 실행하는 것이 좋습니다. 이러한 오류는 조용한
  ...
  fix
	모든 파일과 패리티 데이터를 복구(수정)합니다.

	모든 파일과 패리티 데이터는 마지막 `sync`에 저장된 상태와 비교됩니다.
	차이점이 발견되면 저장된 상태로 되돌립니다.

	경고!
	`fix` 명령어는 오류와 의도적인 수정을 구별하지 않습니다.
	파일 상태를 조건 없이 마지막 `sync` 상태로 되돌립니다.
  ...
  locate
	패리티 디스크에 저장된 파일을 찾습니다. 일치하는 각 파일에 대해
	패리티 파일 내의 위치와 점유하고 있는 조각(fragment)의 수를
	출력합니다.

	-t, --tail 옵션을 사용하여 패리티의 지정된 꼬리(tail) 부분을 점유하는
	파일로 작업을 제한할 수 있습니다.

	이 파일들을 재할당하려면 -W, --force-realloc-tail 옵션을
	사용할 수 있습니다.
...
옵션 (Options)
	...
	-e, --filter-error
		...
		`status`에 나열된 대로 `sync` 및 `scrub` 중에 조용한 오류 또는 입출력 오류가
	...
	-o, --older-than DAYS
		...
		DAYS는 블록이 스크럽되어야 하는 최소 경과 일수(days)이며,
		기본값은 10입니다.
	...
	-h, --pre-hash
		...
		데이터가 아직 해시되지 않았기 때문에 조용한 오류가 감지되지 않고
		지나갈 수 있습니다.
	...
	-t, --tail SIZE
		패리티 디스크의 지정된 꼬리 크기 이하를 사용하는 파일로
		목록을 제한합니다.
		K, M, G, T와 같은 배수를 사용할 수 있습니다 (예: --tail 1G).
		이 옵션은 `locate` 명령과 함께 사용할 때만 유효합니다.
	...
	-N, --force-nocopy
		...
		식별하고, 이미 계산된 해시 정보를 재사용하여 조용한 오류를
	...
	-W, --force-realloc-tail SIZE
		-R, --force-realloc과 같이 작동하지만, 각 패리티 파일의 지정된
		꼬리 부분(마지막 '크기' 바이트)으로 제한됩니다.
		현재 해당 꼬리 섹션에 저장된 모든 파일 조각/블록의 재할당(이동)을
		강제하여, 여유 공간이 있는 패리티 파일 내의 어느 곳이든(기존
		빈 공간 포함) 배치될 수 있도록 합니다.
		이 옵션의 주요 목적은 패리티 파일의 디스크상 크기를 줄이는 것입니다.
		재할당을 통해 전체 꼬리 섹션이 성공적으로 비워지면(남은 블록이 없음),
		패리티 파일이 잘리고(truncated) 사용되지 않는 꼬리 공간을 회수합니다.
		K, M, G, T와 같은 배수를 사용할 수 있습니다 (예:
		--force-realloc-tail 1G).
		locate -t, --tail을 사용하여 영향을 받는 파일을 사전에
		확인할 수 있습니다.
		경고!
		이 옵션은 전문가 전용이며, 사용하지 않을 것을 강력히
		권장합니다.
		영향을 받는 파일에 대해서는 `sync` 작업 동안 데이터 보호를
		받을 수 없습니다.
	...
	-S, --start BLKSTART
		...
		유용할 수 있습니다.
		이 옵션은 주로 고급 수동 복구를 위한 것이며 `check` 및 `fix`에서만
		지원됩니다.
...
설정 (Configuration)
  ...
  parity FILE [,FILE] ...
	...
	총 사용 가능한 공간은 어레이에서 가장 큰 데이터 디스크의 크기
	이상이어야 합니다.
  ...
  data NAME DIR
	...
	그러나 구성된 경로는 항상 동일한 기본 데이터로 확인되어야 합니다.
	경로에 심볼릭 링크가 포함된 경우 해당 대상이 안정적으로 유지되어야 합니다.
	데이터 디렉터리를 즉석에서 전환하기 위해 이러한 심볼릭 링크의 대상을
	변경하는 것은 지원되지 않습니다.
  ...
  extra NAME DIR
	`smart` 및 `probe` 명령으로 모니터링할 추가 디스크의 이름과 마운트
	지점을 정의합니다.

	이것은 부팅 디스크와 같이 어레이의 일부는 아니지만 시스템 작동에 필요
	한 디스크를 모니터링하는 데 유용합니다.

	이러한 디스크는 항상 회전할 것으로 예상되므로 `up` 및 `down` 명령의
	영향을 받지 않습니다.

  snapshot
	`sync`, `scrub`, `check` 및 `fix` 명령어에 파일시스템 스냅샷 사용을
	활성화합니다.

	활성화되면 SnapRAID는 `sync` 시작 시 지원되는 각 데이터 파일시스템의
	읽기 전용 스냅샷을 생성합니다. 이는 각 파일시스템의 안정적인 뷰를 제공하여,
	이후의 수정으로 인해 패리티 계산에 사용되는 데이터가 변경되는 것을 방지합니다.

	이는 복구 가능성을 크게 향상시킵니다. 라이브 파일시스템에서 파일이 삭제되더라도
	스냅샷에 그대로 보존됩니다. 이를 통해 해당 블록의 패리티가 "깨지는" 것을 방지하여,
	다른 디스크가 고장 나더라도 데이터를 성공적으로 복구할 수 있도록 보장합니다.

	이 옵션은 Linux의 Btrfs, Bcachefs 또는 ZFS 파일시스템과 Windows의 NTFS로 포맷된
	데이터 디스크에만 독점적으로 적용됩니다.
	패리티 디스크나 다른 파일시스템을 사용하는 데이터 디스크는 항상 파일시스템의
	라이브 버전을 사용합니다.

	스냅샷 생성 및 삭제에는 관리자 권한이 필요합니다.
	이 옵션이 활성화된 경우 SnapRAID가 필요한 권한(예: sudo)으로 실행되는지 확인하십시오.

	스냅샷 수명 주기에 대한 자세한 설명은 SNAPSHOTS 섹션을 참조하십시오.
  ...
  exclude/include PATTERN
	...
	필터링은 디렉토리 순회 중에 계층적으로 수행됩니다. 디렉토리가 `exclude`
	규칙과 일치하면 해당 디렉토리 전체가 건너뛰어지며(가지치기됨), 그 안에
	있는 파일이나 하위 디렉토리에 대한 `include` 규칙은 평가되지 않습니다.
  ...
  temp_limit TEMPERATURE_CELSIUS
	...
	도달할 것으로 예상되는 장기 평형 온도를 추정합니다.
	추정은 디스크 온도가 네 번 증가한 후에만 수행되어, 신뢰할 수 있는
	추세를 설정하기에 충분한 데이터 포인트가 확보되도록 합니다.
	이 예측된 평형 온도는 현재 값 옆에 괄호 안에 표시되며, 시스템의
	냉각이 적절한지 평가하는 데 도움이 됩니다. 이 추정 평형 온도는 정보 제공
  ...
  smartignore DISK/PARITY ATTR [ATTR...]
	...
	여러 속성을 공백으로 구분하여 지정할 수 있습니다.
	각 속성은 1에서 255 사이의 숫자나 대소문자를 구분하지 않는 이름
	(예: `Current_Pending_Sector`)으로 지정할 수 있습니다.

	예를 들어, 모든 디스크에서 `Current Pending Sector Count` 와
	`Reallocated Sectors Count` 속성을 번호나 이름으로 무시하려면:
...
스냅샷 (Snapshots)
	구성에서 스냅샷 옵션이 활성화된 경우, SnapRAID는 작업 중에 읽은 데이터를
	안정적으로 유지하기 위해 파일시스템 스냅샷을 사용하며, 이는 라이브 파일시스템의
	파일이 이후에 수정되거나 삭제되는 경우에도 마찬가지입니다.

	스냅샷 관리는 완전히 자동적이고 투명하게 이루어집니다.
	백그라운드에서 완전히 처리되는 스냅샷의 추가적인 보호와 함께,
	기존과 완전히 동일하게 SnapRAID를 계속 사용할 수 있습니다.

	이것은 두 가지 주요 이점을 제공합니다.

	일관성 - 장시간 실행되는 sync 또는 scrub 중에 라이브 파일시스템에서 수정된 파일은
		스냅샷이 생성된 각 파일시스템이 작업 기간 동안 변경되지 않고 유지되므로
		패리티 불일치나 작업 중단을 유발하지 않습니다.
	복구 - 라이브 파일시스템에서 파일이 업데이트되거나 삭제되더라도 스냅샷에 보존됩니다.
		다음 sync가 실행되기 전에 디스크 고장이 발생하면, SnapRAID는 스냅샷에 보존된
		데이터를 사용하여 고장 난 디스크를 재구성합니다.

		활성 sync 프로세스 중에 디스크 고장이 발생하는 경우,
		SnapRAID는 이전 패리티 계산의 스냅샷과 현재 스냅샷 모두에서 자동으로 읽을 수도 있습니다.
		이는 동기화 사이에 블록이 수정되거나 삭제되었더라도 패리티 방정식을 푸는 데
		필요한 정확한 데이터 블록에 대한 접근을 제공함으로써 완전한 복구 확률을 최대화합니다.

		스냅샷이 없으면 정상 디스크에서 업데이트되거나 삭제된 파일로 인해 데이터 블록이 누락되며,
		이는 다른 고장 난 디스크를 복구하는 데 필요할 수 있습니다.

	스냅샷은 데이터 디스크에 대해서만 생성되며, 기본 파일시스템이 이 기능을 지원하는 경우에만
	생성됩니다. 패리티 디스크는 항상 라이브 파일시스템을 사용합니다.

	현재 이는 Linux의 Btrfs, Bcachefs, ZFS 및 Windows의 NTFS에서 지원됩니다.

	서로 다른 파일시스템을 가진 데이터 디스크를 혼합할 수 있습니다. 스냅샷을 지원하는 디스크만
	이를 활용하고, 다른 디스크는 라이브 파일시스템에서 직접 계속 작동합니다.

	스냅샷 생성 및 삭제에는 관리자 권한이 필요합니다.
	스냅샷이 활성화된 경우 SnapRAID가 필요한 권한(예: sudo)으로 실행되는지 확인하십시오.

  스냅샷 사용 시 명령어 동작 (Command Behavior with Snapshots)
	`sync` 및 `scrub` 명령어는 파일시스템이 스냅샷을 지원하는 데이터 디스크에 대해
	스냅샷을 사용합니다. 스냅샷이 생성된 디스크에서 명령어는 라이브 파일시스템의
	동시 변경에 영향을 받지 않는 안정적인 파일시스템 이미지를 봅니다.
	`sync` 중에 지원되는 각 데이터 파일시스템에 대해 독립적으로 새 스냅샷이 생성됩니다.
	이러한 스냅샷은 집합적으로 패리티 계산에 사용되는 데이터를 제공합니다.
	반면에 `scrub` 명령어는 기존 패리티와 일치하는 신뢰할 수 있는 참조 지점을 유지하기 위해
	가장 최근 동기화 중에 생성된 마지막 스냅샷을 이용합니다.

	`check` 및 `fix` 명령어의 경우, 마지막 스냅샷의 사용 여부는
	-d, --filter-disk 옵션을 사용하여 특정 디스크를 대상으로 지정하는지 여부에 따라 달라집니다.

	-d를 통해 명시적으로 선택된 디스크(작업의 "대상")는 항상 라이브 파일시스템을 사용합니다.
	`fix` 의 경우 이를 통해 활성 교체 디스크에 데이터를 복원할 수 있습니다.
	`check` 의 경우 동일한 조건에서 `fix` 작업을 시뮬레이션할 수 있습니다.

	다른 모든 데이터 디스크("참조" 디스크)는 해당 스냅샷을 통해 액세스됩니다.
	이를 통해 복구가 진행되는 동안 정상 디스크의 파일을 수정하고 있더라도 SnapRAID가
	패리티 방정식을 풀기 위한 안정적이고 동결된 참조를 가질 수 있습니다.

	-d 옵션이 제공되지 않으면 SnapRAID는 작업이 전체 어레이에 적용된다고 가정하며, 이 경우
	`check` 와 `fix` 는 라이브 파일시스템만을 독점적으로 사용합니다.

	다른 모든 명령어는 라이브 파일시스템에서만 독점적으로 작동합니다.

스냅샷 수명 주기 (Snapshots Lifecycle)
	SnapRAID는 각 데이터 하위 볼륨의 루트에 있는 숨겨진 디렉터리 내에서
	`stable`(안정), `pending`(보류) 및 `scan`(스캔)이라는 세 가지 특정 스냅샷을 관리합니다.
	Btrfs, Bcachefs 및 NTFS에서는 `.snapraid/` 디렉터리가 사용되며,
	ZFS에서는 표준 `.zfs/snapshot/` 이 사용됩니다.

	`stable` 스냅샷은 성공적으로 완료된 마지막 `sync` 상태를 나타내며,
	현재 패리티를 계산하는 데 사용된 정확한 데이터를 포함합니다. 이는
	`scrub`, `check` 및 `fix` 명령어의 기본 데이터 소스 역할을 합니다.

	`scan` 스냅샷은 검사 및 패리티 계산을 위한 동결된 상태를 제공하기 위해 `sync` 시작 시
	생성되는 임시 이미지입니다.
	검사를 수행하고 새 콘텐츠 상태를 저장한 후 이전 `pending` 스냅샷을 대체합니다.

	`pending` 스냅샷은 중단된 `sync` 에서 사용하는 이미지입니다.
	작업이 성공적으로 완료되면 이전의 `stable` 스냅샷은 삭제되고, `pending` 스냅샷이
	새로운 안정적인 참조로 자리를 잡도록 승격됩니다.

	`sync` 가 중단되면 `pending` 스냅샷은 보존됩니다.
	패리티가 부분적으로만 동기화되었더라도 .content 파일에 기록된 파일시스템 상태와 일치하므로
	이후의 `scrub`, `check` 및 `fix` 명령어는 이 보류 중인 스냅샷을 사용합니다.

	`sync` 가 중단된 경우, `check` 및 `fix` 명령어는 `stable` 스냅샷에서도 읽어와
	동기화 중에 업데이트되거나 삭제된 파일에서 데이터 블록을 검색합니다.
	이는 동기화 프로세스 전반에 걸쳐 .content 파일이 수정되거나 삭제된 모든 파일의
	메타데이터를 유지하기 때문에 가능합니다.
	이 기록 정보는 동기화가 성공적으로 완료되고 .content 파일의 최종 버전이 저장되어야만 삭제됩니다.

	중단 후 `sync` 가 재시작되면 기존의 보류 중인(pending) 스냅샷은 삭제되고,
	라이브 파일시스템의 현재 상태를 캡처하기 위해 새 스냅샷이 생성됩니다.
	그러나 `sync` 가 중단된 경우 후속 `sync` 가 성공적으로 완료될 때까지
	파일을 수정, 삭제, 교체 또는 이동하지 마십시오.
	패리티에는 중단된 sync에서 사용된 스냅샷에서 계산된 블록이
	여전히 포함되어 있을 수 있습니다. 이후의 sync로 인해 해당 스냅샷이
	교체되어 복구에 필요한 중간 버전의 파일을 사용할 수 없게 될 수 있습니다.
...
패턴 (Pattern)
	...
	이중 별표 와일드카드 `**`는 디렉토리 구분 기호에 인접해 있는 경우(슬래시
	앞이나 뒤에 오는 경우)에만 디렉토리 구분 기호를 가로질러 일치합니다.
	이 형식에서는 여러 디렉토리 수준에 걸쳐 0개 이상의 문자와 일치합니다.
	예를 들어, `foo/**/bar`는 `foo/bar`, `foo/a/bar` 및 `foo/a/b/bar`와 일치하고,
	`**/build`는 `build` 및 `foo/bar/build`와 일치하며, `src/**.js`는 `src/main.js` 및
	`src/ui/components/main.js`와 일치합니다.
	독립형 형식인 `/**/`는 완전한 패턴으로 허용되지 않습니다.

	`**`가 슬래시에 인접하지 않은 경우 `*`처럼 작동하며 디렉토리 구분 기호와
	일치하지 않습니다. 예를 들어, `**.jpg`는 현재 디렉토리의 `photo.jpg`와는
	일치하지만 `dir/photo.jpg`와는 일치하지 않습니다. 해당 용도로는 `**/*.jpg`를
	사용하십시오.
	...
	`**`는 슬래시에 인접해 있는 경우에만 디렉토리 구분 기호를 가로질러
	일치할 수 있다는 것입니다.
	...
	디렉토리를 제외하면(DIR/ 또는 /PATH/DIR/ 사용) SnapRAID가 해당 디렉토리에
	진입하여 스캔하는 것이 차단됩니다. 따라서 제외된 상위 디렉토리 내에 있는
	파일이나 하위 디렉토리는 스캐너가 탐색하지 않으므로 `include` 규칙으로 다시
	포함할 수 없습니다.

	예를 들어, 다음 설정에서는 `/keep/`이 가지치기되므로 `file.dat`가 포함되지
	않습니다:

		:# 의도대로 작동하지 않음: /keep/ 전체를 건너뜀
		:include /keep/file.dat
		:exclude /keep/

	디렉토리 자체를 제외하지 않고 특정 파일만 유지하면서 내부 파일들을 제외하려면,
	디렉토리 자체가 아니라 내부의 파일 패턴을 제외하십시오:

		:# 의도대로 작동함: /keep/을 순회하지만 내부의 다른 파일들은 제외됨
		:include /keep/file.dat
		:exclude /keep/*
...
콘텐츠 (Content)
	...
	content 파일은 `sync`, `scrub`, `touch` 및 `rehash` 명령에 의해
	읽고 쓰여집니다. 이것은 `status`, `diff`, `check`, `fix`, `list`,
...
종료 코드 (Exit Code)
	SnapRAID는 다음과 같은 오류 코드로 종료됩니다:

	0 - 모두 정상.
	1 - 명령 실행 중 오류가 발생했습니다.
	2 - `diff` 명령으로 모두 정상임을 확인했으나 `sync`가 필요합니다.
	3 - 디스크 장치 명령(`probe`, `smart`, `devices`, `up`, `down`)이 완료되었으나,
		누락된 장치로 인해 하나 이상의 파일 시스템이 저하(degraded) 모드로 실행 중입니다.

알려진 문제 (Known Issues)
  파일 시스템 경계
	SnapRAID는 데이터 디스크를 스캔하는 동안 파일 시스템, 마운트 또는 서브볼륨 경계를
	넘지 않습니다. 이러한 경계가 감지되면 SnapRAID는 경고(WARNING)를 보고하고
	그 너머를 스캔하지 않습니다.

	이러한 경계 너머에 있는 파일은 데이터 디스크에 포함되지 않으며, 보호해야 하는 경우
	별도로 구성해야 합니다.

  파일 시스템 스냅샷 사용 시 마운트 지점
	파일 시스템 스냅샷이 활성화된 경우, 데이터 디스크 내부의 중첩된 마운트 지점 및
	바인드 마운트는 지원되지 않습니다.

	마운트 지점은 라이브 시스템 마운트 네임스페이스의 일부이며 파일 시스템 스냅샷에
	보존되지 않습니다. 따라서 스냅샷을 스캔할 때 라이브 파일 시스템에서 마운트 지점에
	의해 숨겨졌던 디렉터리가 보일 수 있습니다.

	파일 시스템 스냅샷이 활성화된 경우 데이터 디스크 내에 중첩된 마운트 지점이나
	바인드 마운트를 배치하지 마십시오.

  파일 시스템 스냅샷 사용 시 여러 설정
	동일한 파일 시스템 스냅샷 네임스페이스를 사용하는 여러 독립된 SnapRAID
	설정 간에는 스냅샷 지원을 안전하게 공유할 수 없습니다.

	Btrfs 및 Bcachefs에서는 스냅샷 지원이 활성화된 단 하나의 SnapRAID 설정만이
	동일한 서브볼륨 내의 데이터 디렉터리를 사용할 수 있습니다. ZFS에서는 동일한
	제한이 데이터셋에 적용되며, NTFS에서는 볼륨에 적용됩니다.

	동일한 스냅샷 네임스페이스를 공유하는 여러 설정은 동일한 `stable`, `pending`
	및 `scan` 스냅샷을 사용합니다. 따라서 하나의 설정을 실행하면 다른 설정에 필요한
	복구 스냅샷을 대체하거나 삭제할 수 있습니다.

  타임스탬프 변경 없는 인플레이스 파일 수정
	SnapRAID는 파일 크기와 수정 타임스탬프(mtime)를 비교하여 파일 수정을 감지합니다.

	애플리케이션이 파일 크기를 변경하지 않고 수정 타임스탬프도 업데이트하지 않은 채
	인플레이스로 파일을 수정하는 경우, "diff" 및 "sync"는 파일이 변경되었음을 감지하지
	못하며 패리티가 업데이트되지 않습니다.

	SnapRAID가 이러한 변경 사항을 인식하도록 하려면 파일의 수정 타임스탬프를
	업데이트하십시오(예: "touch" 유틸리티 사용). 그렇지 않으면 이 차이점은 이후의
	"scrub" 또는 "check" 중에 해시 불일치로만 감지됩니다.

  Windows 비ASCII 대소문자 일치
	SnapRAID는 모든 플랫폼에서 표준적이고 예측 가능한 UTF-8 인코딩을 사용하며,
	대소문자를 구분하지 않는 일치를 ASCII 문자로 제한합니다.

	Windows 파일 시스템은 임의의 시스템 종속 폴딩 테이블을 통해 유니코드 파일 이름
	전체에서 대소문자 구분을 없애도록 강제하는 레거시 디자인의 한계를 안고 있습니다.
	결과적으로 Windows는 완전히 다른 유니코드 바이트 시퀀스를 마치 동일한 파일인 것처럼
	취급하여 파일 시스템의 실제 상태를 왜곡합니다.

	예를 들어 `À.txt`와 `à.txt`는 고유한 유니코드 이름이지만, Windows는 비논리적으로
	이를 동일하게 취급하는 반면 SnapRAID는 이들의 진정한 차이를 보존합니다.

	이 제한은 Windows의 일관성 없는 유니코드 파일 이름 처리와 대소문자를 구분하지 않는
	별칭 지정 고집으로 인해 발생합니다. Windows의 대소문자 폴딩으로 인한 불일치를
	방지하려면 구성 경로 및 필터의 비ASCII 문자에 대해 항상 정확한 대소문자를
	지정하십시오.

  부분 fix 복구
	`fix`가 `-S, --start` 또는 `-B, --count`를 사용하여 제한되면,
	SnapRAID는 선택된 블록에서만 작동하며 파일 수준의 완료 처리나 격리를
	수행하지 않습니다.

	복구 결과 현재 내용을 나타낸다고 입증할 수 없는 최선의(best-effort) 데이터가
	생성되더라도, 이러한 블록은 일반 파일 이름으로 기록될 수 있습니다. 이러한
	복구된 블록과 관련된 불확실성은 명령이 종료된 후 유지되지 않습니다.

	결과적으로 이후의 `fix`는 이러한 읽기 가능한 블록을 유효한 파일 데이터로
	취급하고 다시 복구하려고 시도하지 않습니다. 이로 인해 파일에 불완전하거나
	오래된 데이터가 남아 있음에도 후속 복구가 성공으로 보고될 수 있습니다.

	`-S` 및 `-B` 옵션은 일반적으로 일반 `fix`가 실패한 후 고급 수동 복구만을
	위한 것입니다. 부분 `fix`에서 복구할 수 없는 데이터가 보고되면 다른 복구를
	시도하기 전에 영향을 받는 파일을 제거하거나 이름을 바꾸거나 해당 내용을
	수동으로 확인하십시오.

  NTFS 하드 링크 및 제외된 경로
	NTFS에서는 디렉터리 항목이 각 하드 링크의 파일 크기와 타임스탬프를
	독립적으로 캐시합니다. 한 링크를 통해 파일이 수정되면 동일한 파일을
	가리키는 다른 링크의 디렉터리 항목은 액세스될 때까지 오래된 메타데이터를
	유지할 수 있습니다.

	SnapRAID는 스캔 중에 동일한 파일에 대한 하드 링크를 2개 이상 발견하면
	이러한 상태를 자동으로 감지하고 정규화합니다. 그러나 다른 링크가 필터에
	의해 제외되었거나 데이터 디스크 경로 외부에 있어 하나의 하드 링크만
	스캔되는 경우, SnapRAID는 inode 충돌을 감지할 수 없으며 표시되는 유일한
	링크의 캐시된 메타데이터에 의존합니다.

	결과적으로 제외되었거나 외부 하드 링크를 통해 변경된 사항은 해당 경로를
	통해 파일에 액세스하거나 열어 운영 체제에서 포함된 링크의 메타데이터를
	새로 고칠 때까지 SnapRAID에서 감지되지 않을 수 있습니다.

	이러한 제한을 방지하려면 SnapRAID에서 제외된 하드 링크를 통해 보호된
	파일을 수정하지 않거나, SnapRAID가 이를 정규화할 수 있도록 파일의 모든
	하드 링크가 스캔에 포함되도록 하십시오.

  중단된 다중 패리티 sync 후 복구
	다중 패리티 디스크가 있는 어레이에서 `sync`가 중단되면 서로 다른
	패리티 레벨이 일시적으로 동기화되지 않은 상태로 남을 수 있습니다.
	예를 들어 패리티 1에는 새로 업데이트된 데이터가 이미 포함되어 있는 반면,
	패리티 2에는 여전히 이전 데이터가 남아 있을 수 있습니다.

	`check` 또는 `fix`를 수행하는 동안 SnapRAID는 다양한 패리티 조합을
	시도하고 알려진 블록 해시(전체 블록을 포함하며 일반적으로 256 KiB)에 대해
	재구성된 데이터를 검증합니다.

	SnapRAID가 이전 패리티와 새 패리티의 일치하지 않는 혼합을 사용하여
	재구성을 시도하면 선형 RAID 방정식은 잘못된 데이터를 생성합니다. 이러한
	유효하지 않은 조합이 잘못 수락되려면, 동일한 스트라이프의 다른 비동기화된
	블록이 잘못된 데이터로 재구성되는 와중에도 방정식이 우연히 하나 이상의
	완전한 256 KiB 블록을 완벽하게 재구성하여 해시가 일치해야 합니다.

	이 시나리오는 해시 충돌에 의존하지 않습니다. 오히려 혼합된 패리티
	방정식 자체가 우연히 원래의 정확한 256 KiB 블록 내용을 생성해야 합니다.
	이는 특별히 조작되었거나 매우 반복적인 데이터에서는 수학적으로 가능하지만,
	실제 데이터에서는 사실상 불가능합니다.

	모든 디스크가 정상인 경우 추가 변경을 수행하기 전에 중단된 `sync`를
	완료하면 이러한 불확실성이 제거됩니다. 디스크에 오류가 발생하여 복구가
	필요한 경우 `check` 또는 `fix`를 실행하기 전에 데이터 디스크를 수정하지
	마십시오.

  예약된 파일 및 디렉터리 이름
	SnapRAID는 내부 작업을 위해 일부 파일, 디렉터리 및 스냅샷 이름을
	예약합니다. 사용자 데이터에 이러한 이름을 사용하지 마십시오:

	.snapraidignore - 로컬 제외 규칙 파일.
	.snapraid - 스냅샷 컨테이너 디렉터리.
	snapraid-* - SnapRAID에서 관리하는 ZFS 스냅샷 이름.
	*.unrecoverable - 복구 중 격리된 손상된 파일.
	CONTENT.tmp - content 업데이트용 임시 파일.
	CONTENT.lock - content 잠금 파일.
	PARITY.spaceholder - Windows 패리티 할당 중 사용되는 임시
		공간 확보 파일.

	관련 없는 데이터에 이러한 이름을 사용하면 파일이나 스냅샷이 무시되거나,
	메타데이터로 잘못 해석되거나, 덮어쓰기되거나, 이름이 바뀌거나, 삭제될 수
	있습니다.

	Windows에서는 대소문자를 구분하도록 구성된 NTFS 디렉터리에서도 모든
	대소문자 변형이 마찬가지로 예약됩니다(예: .SNAPRAIDIGNORE, .SnapRaid,
	CONTENT.TMP, CONTENT.Lock).
