...
Pattern
	패턴은 처리에서 제외하거나 포함할 파일의 하위 집합을 선택하는 데
	사용됩니다.

	네 가지 유형의 패턴이 있습니다:

	=FILE
		FILE이라는 이름의 모든 파일을 선택합니다. *, ?와 같은 글로빙
		문자와 [a-z]와 같은 문자 클래스를 사용할 수 있습니다.
		이 패턴은 디렉토리가 아닌 파일에만 적용됩니다.

	=DIR/
		DIR이라는 이름의 모든 디렉토리와 그 안의 모든 것을 선택합니다.
		*, ?와 같은 글로빙 문자를 사용할 수 있습니다.
		이 패턴은 파일이 아닌 디렉토리에만 적용됩니다.

	=/PATH/FILE
		정확히 지정된 파일 경로를 선택합니다. *, ?와 같은 글로빙
		문자를 사용할 수 있지만, 디렉토리 슬래시와는 절대 일치하지 않습니다.
		이 패턴은 디렉토리가 아닌 파일에만 적용됩니다.

	=/PATH/DIR/
		정확히 지정된 디렉토리 경로와 그 안의 모든 것을 선택합니다.
		*, ?와 같은 글로빙 문자를 사용할 수 있지만, 디렉토리 슬래시와는
		절대 일치하지 않습니다.
		이 패턴은 파일이 아닌 디렉토리에만 적용됩니다.
