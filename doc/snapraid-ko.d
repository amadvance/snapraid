...
제한 사항 (Limitations)
	...
	* 스냅샷 지원이 없는 경우:
		...
		있습니다. 이는 수정된 파일이 고장 난 디스크에 없더라도 파리티가 수정된 파일과
...
명령어 (Commands)
  ...
  fix
	모든 파일과 파리티 데이터를 복구(수정)합니다.

	모든 파일과 파리티 데이터는 마지막 `sync`에 저장된 상태와 비교됩니다.
...
설정 (Configuration)
  ...
  snapshot
	...
	이는 복구 성능을 크게 향상시킵니다. 라이브 파일시스템에서 파일이 삭제되더라도
	스냅샷에 그대로 보존됩니다. 이를 통해 해당 블록의 파리티가 "깨지는" 것을 방지하여,
	...
	파리티 디스크나 다른 파일시스템을 사용하는 데이터 디스크는 항상 파일시스템의
	라이브 버전을 사용합니다.
...
스냅샷 (Snapshots)
	...
	일관성 - 장시간 실행되는 sync 또는 scrub 중에 라이브 파일시스템에서 수정된 파일은
		SnapRAID가 동결된 시점의 이미지를 보기 때문에 파리티 불일치나 작업 중단을
		유발하지 않습니다.
	복구 - 라이브 파일시스템에서 파일이 업데이트되거나 삭제되더라도 스냅샷에 보존됩니다.
		...
		SnapRAID는 이전 파리티 계산의 스냅샷과 현재 스냅샷 모두에서 자동으로 읽을 수도 있습니다.
		이는 동기화 사이에 블록이 수정되거나 삭제되었더라도 파리티 방정식을 푸는 데
  ...
  스냅샷 사용 시 명령어 동작
	...
	라이브 파일시스템의 동시 파일 수정으로 인해 발생할 수 있는 파리티 불일치를 방지합니다.
	`sync` 중에 명령어는 프로세스 시작 시 생성된 새 스냅샷을 사용하여 파리티 계산을 위한
	현재 상태를 캡처합니다.
	반면에 `scrub` 명령어는 기존 파리티와 일치하는 신뢰할 수 있는 참조 지점을 유지하기 위해
	...
	파리티 방정식을 풀기 위한 안정적이고 동결된 참조를 가질 수 있습니다.
