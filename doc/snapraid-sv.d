...
Begränsningar (Limitations)
	...
	För data som redan existerade vid den senaste synkroniseringen beror återställningens tillförlitlighet
	...
	* Utan stöd för ögonblicksbilder kan borttagning eller ändring av filer efter en `sync`
		förhindra fullständig återställning av andra felaktiga diskar. Detta inträffar
		eftersom pariteten inte längre matchar de modifierade filerna, även om
		dessa filer inte finns på den havererade disken. Å andra sidan förhindrar inte nyinkomna
		filer återställning av redan existerande filer.
...
Kommandon (Commands)
	...
	* Skapa en backup-/återställningspunkt -> `sync`
	* Periodiskt kontrollera data -> `scrub`
	* Återställa den senaste backup-/återställningspunkten -> `fix`.

	Kommandon måste skrivas med gemener (små bokstäver).
  ...
  diff
	...
	grupperade efter:
		...
		updated - Filer med en annan storlek eller tidsstämpel, vilket innebär att de
			modifierades.
		moved - Filer som flyttats eller bytt namn på samma disk med bibehållen
			inode. De identifieras genom att ha samma inode, storlek och
			tidsstämpel, men en annan sökväg. Filnamnet kan också vara annorlunda.
		...
		relocated - Filer som flyttats till en annan katalog eller disk där
			originalfilen har försvunnit och inoden inte bibehålls.
			De identifieras genom att ha samma namn, storlek och tidsstämpel.
			Om tidsstämpeln för undersekunder är noll måste hela sökvägen
			matcha för att identifieras. Till skillnad från 'moved'-filer kräver
			omlokaliserade filer att filnamnet förblir detsamma.
...
Konfiguration (Configuration)
  ...
  data NAME DIR
	...
	Den konfigurerade sökvägen måste dock konsekvent matcha samma
	underliggande data. Om sökvägen innehåller symboliska länkar
	måste deras mål förbli stabila. Att ändra sådana symlänkar för
	att byta datakatalog i farten stöds inte.
  ...
  snapshot
	...
	för det blocket, vilket säkerställer att du fortfarande kan återställa data framgångsrikt
	om en annan disk havererar.

	Detta alternativ gäller uteslutande datadiskar formaterade med filsystemen
	Btrfs, Bcachefs eller ZFS i Linux och NTFS i Windows.
	Paritetsdiskar, eller datadiskar som använder andra filsystem, kommer alltid att använda
	den aktiva versionen av filsystemet.

	Att skapa och ta bort ögonblicksbilder kräver administratörsbehörighet.
	Se till att SnapRAID körs med nödvändiga rättigheter (t.ex. sudo) när
	detta alternativ är aktiverat.

	Se avsnittet SNAPSHOTS för en detaljerad förklaring av ögonblicksbildens livscykel.
  ...
  smartignore DISK/PARITY ATTR [ATTR...]
	...
	Flera attribut kan specificeras separerade med mellanslag.
	Varje attribut kan specificeras som ett nummer från 1 till 255, eller som ett
	...
	För att ignorera dem endast på den första paritetsdisken:
...
Ögonblicksbilder (Snapshots)
	...
	ytterligare skydd som ögonblicksbilder ger, helt hanterat i
	bakgrunden.

	Detta ger två huvudsakliga fördelar:
	...
	Återställning - Om en fil uppdateras eller tas bort från det aktiva filsystemet
		...
		i ögonblicksbilden för att rekonstruera den felaktiga disken.

		I händelse av ett diskhaveri under en aktiv synkroniseringsprocess
		kan SnapRAID också läsa automatiskt från både ögonblicksbilden
		av den föregående paritetsberäkningen och den aktuella.
		Detta maximerar sannolikheten för en fullständig återställning genom att
		ge tillgång till de exakta datablock som krävs för att lösa paritetsekvationerna,
		även om dessa block modifierades eller togs bort mellan synkroniseringar.

		Utan ögonblicksbilder leder en uppdaterad eller borttagen fil på en frisk disk
		till saknade datablock som kan krävas för att laga andra felaktiga diskar.
	...
	Du kan blanda datadiskar med olika filsystem. Endast de som
	stöder ögonblicksbilder kommer att använda dem, medan andra diskar kommer att fortsätta
	att fungera direkt på det aktiva filsystemet.

	Att skapa och ta bort ögonblicksbilder kräver administratörsbehörighet.
	Se till att SnapRAID körs med nödvändiga rättigheter (t.ex. sudo)
  ...
  Kommandobeteende med ögonblicksbilder
	...
	För kommandona `check` och `fix` beror användningen av den senaste ögonblicksbilden
	på om specifika diskar är måltavla med alternativet
	-d, --filter-disk.

	En disk som uttryckligen valts via -d (operationens "mål")
	använder alltid det aktiva filsystemet. För `fix` tillåter detta återställning
	av data till den aktiva ersättningsdisken. För `check` tillåter det att simulera
	`fix`-operationen under samma förhållanden.

	Alla andra datadiskar ("referensdiskarna") kommer att nås via
	sina ögonblicksbilder. Detta säkerställer att även om du ändrar filer
...
Livscykel för ögonblicksbilder (Snapshots Lifecycle)
	...
	Ögonblicksbilden `stable` representerar tillståndet för den senaste
	framgångsrikt slutförda `sync`-iseringen, och innehåller exakt den data som användes för
	...
	Om en `sync` avbryts bevaras ögonblicksbilden `pending`.
	Efterföljande `scrub`-, `check`- och `fix`-kommandon kommer att använda denna
	vilande ögonblicksbild eftersom den matchar filsystemets tillstånd som registrerats i
	.content-filen, även om pariteten endast är delvis synkroniserad.

	I händelse av ett avbrott i en `sync` läser kommandona `check` och `fix`
	också från ögonblicksbilden `stable` för att hämta datablock
	från filer som uppdaterades eller togs bort under synkroniseringen.
	Detta är möjligt eftersom .content-filen behåller metadata för
	alla modifierade eller borttagna filer under hela synkroniseringsprocessen.
	Denna historiska information rensas först när en sync slutförts
	framgångsrikt och den slutliga versionen av .content-filen sparats.
...
Kända problem (Known Issues)
  ...
  Återställning efter ett avbrutet multi-paritets-sync
	I en array med flera paritetsdiskar kan ett avbrutet `sync` lämna
	paritetsnivåerna tillfälligt osynkroniserade med varandra. Till exempel
	kan paritet 1 redan innehålla nyligen uppdaterade data, medan paritet 2
	fortfarande innehåller gamla data.

	Under `check` eller `fix` provar SnapRAID olika paritetskombinationer och
	validerar rekonstruerade data mot kända blockhashar (som täcker hela
	blocket, vanligtvis 256 KiB).

	Om SnapRAID försöker en rekonstruktion med en inkonsekvent blandning av
	gammal och ny paritet ger de linjära RAID-ekvationerna felaktiga data.
	För att en sådan ogiltig kombination felaktigt ska accepteras måste
	ekvationerna av en slump rekonstruera minst ett helt 256 KiB-block så
	perfekt att dess hash matchar, medan andra osynkroniserade block i samma
	stripe rekonstrueras med felaktiga data.

	Detta scenario beror inte på en hashkollision; snarare måste ekvationerna
	med blandad paritet i sig råka producera det exakta ursprungliga
	innehållet i 256 KiB-blocket. Även om detta är matematiskt tänkbart med
	specialkonstruerade eller mycket repetitiva data, är det praktiskt taget
	omöjligt med verkliga data.

	Om alla diskar är felfria elimineras denna tvetydighet genom att slutföra
	det avbrutna `sync` innan fler ändringar görs. Om en disk har gått sönder
