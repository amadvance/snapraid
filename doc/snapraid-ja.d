...
制限事項 (Limitations)
	...
	* スナップショットサポートがある場合（Btrfs、ZFS、Bcachefs、およびNTFSで利用可能）：
		SnapRAIDはデータディスクの凍結されたリファレンスを自動的に維持できます。
...
設定 (Configuration)
  ...
  snapshot
	...
	有効にすると、SnapRAIDは `sync` の開始時にデータの読み取り専用スナップショットを作成します。
	これにより、ファイルの整合性を保った特定の時点のビューが確保され、ファイルの同時変更によって
	発生するエラーを防ぎます。
...
スナップショット (Snapshots)
	...
	整合性 - 長時間かかるsyncやscrubの実行中にライブファイルシステム上でファイルが変更されても、
		SnapRAIDは凍結された時点のイメージを参照するため、パリティの不一致や処理の中断は発生しません。
  ...
  スナップショット使用時のコマンドの挙動 (Command Behavior with Snapshots)
	`sync` と `scrub` コマンドは、すべてのデータ操作が一貫した凍結状態に対して行われることを
	保証するために、いずれもスナップショット上でのみ動作します。これらのスナップショットを利用することで、
	SnapRAIDはライブファイルシステム上での同時ファイル変更によってトリガーされるパリティの不一致を防ぎます。
	`sync` の実行中、コマンドはプロセスの開始時に作成された新しいスナップショットを使用して、パリティ計算の
	ための現在の状態をキャプチャします。
...
スナップショットのライフサイクル (Snapshots Lifecycle)
	...
	`pending` スナップショットは、パリティ計算のための凍結された状態を提供するために、
