...
Pattern
	パターンは、プロセスで除外または含めるファイルのサブセットを選択するために使用されます。

	4つの異なる種類のパターンがあります。

	=FILE
		FILE という名前の任意のファイルを選択します。* や ? などのグロブ文字、
		および [a-z] などの文字クラスを使用できます。
		このパターンは**ファイルのみ**に適用され、ディレクトリには適用されません。

	=DIR/
		DIR という名前の任意のディレクトリと、その中のすべてを選択します。
		* や ? などのグロブ文字を使用できます。
		このパターンは**ディレクトリのみ**に適用され、ファイルには適用されません。

	=/PATH/FILE
		正確に指定されたファイルパスを選択します。* や ? などのグロブ文字を
		使用できますが、**ディレクトリスラッシュには一致しません**。
		このパターンは**ファイルのみ**に適用され、ディレクトリには適用されません。

	=/PATH/DIR/
		正確に指定されたディレクトリパスと、その中のすべてを選択します。* や ? などの
		グロブ文字を使用できますが、**ディレクトリスラッシュには一致しません**。
		このパターンは**ディレクトリのみ**に適用され、ファイルには適用されません。
