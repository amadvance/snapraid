...
限制 (Limitations)
	...
	主要的限制是，如果磁盘发生故障，您只能将数据恢复到上一次 `sync`（同步）操作的状态。
	自上次同步以来添加的任何数据或进行的任何修改（如果位于发生故障的磁盘上）都将丢失。

	因此，SnapRAID 主要适用于很少更改的数据。

	对于上次同步时已经存在的数据，恢复的可靠性取决于是否使用了 `snapshot`（快照）选项：

	* 具有快照支持（在 Btrfs、ZFS、Bcachefs 和 NTFS 上可用），
		SnapRAID 可以自动维护每个受支持的数据文件系统的独立冻结引用。
		这确保了即使您在同步期间或之后在活动文件系统上修改或删除文件，
		恢复过程仍保持一致且可靠。

	* 在没有快照支持的情况下，在 `sync` 之后删除或更改文件
		可能会阻止其他损坏磁盘的完全恢复。发生这种情况
		是因为校验数据不再与修改后的文件匹配，即使
		这些文件不在损坏的磁盘上。另一方面，新添加的文件
		不会阻止已存在文件的恢复。
...
入门 (Getting Started)
  ...
  合并池 (Pooling)
	...
	Mergerfs 提供了一种更灵活、更高效的方式，将多个驱动器汇集到一个
...
命令 (Commands)
  ...
  status
	...
	它包括有关奇偶校验碎片、块未经检查有多久，以及在数据擦洗 (scrubbing)
	期间遇到的所有记录的静默错误的信息。
  ...
  diff
	...
	列出所有更改的文件后，会显示更改的摘要，按以下方式分组：
		...
		moved - 在同一磁盘上移动或重命名并保留相同 inode 的文件。
			它们通过具有相同的 inode、大小和时间戳，但路径不同来识别。
			文件名也可以不同。
		copied - 在相同或不同磁盘上复制的文件，且原始文件仍然存在。
			它们通过具有相同的名称、大小和时间戳进行识别。
			如果亚秒级时间戳为零，则必须完整路径匹配才能识别，而不仅仅是名
			称。
		relocated - 移动到不同目录或磁盘的文件，且原始文件已消失并不保留 inode。
			它们通过具有相同的名称、大小和时间戳进行识别。
			如果亚秒级时间戳为零，则必须完整路径匹配才能识别。
			与 'moved' 文件不同，重定位的文件要求文件名保持不变。
  ...
  fix
	...
	将所有文件和校验数据与上一次 `sync` 中保存的状态进行比较。
	如果发现差异，则将其恢复到存储的状态。

	警告！
	`fix` 命令不区分错误和故意修改。
	它会无条件地将文件状态恢复到上一次 `sync` 的状态。
...
选项 (Options)
	...
	-S, --start BLKSTART
		...
		此选项主要用于高级手动恢复，并且仅受 `check` 和 `fix` 支持。
...
配置 (Configuration)
  ...
  data NAME DIR
	...
	但是，配置的路径必须始终解析为相同的基础数据。如果路径包含符号链接，
	其目标必须保持稳定。不支持重定向此类符号链接以在运行中切换数据目录。
  ...
  snapshot
	为 `sync`、`scrub`、`check` 和 `fix` 命令启用文件系统快照的使用。

	启用后，SnapRAID 会在“sync”开始时为每个受支持的数据文件系统创建
	只读快照。这提供了每个文件系统的稳定视图，防止后续修改改变用于
	计算校验的数据。

	这显著提高了恢复能力：如果从实时文件系统中删除了一个文件，
	它仍会保留在快照中。这可以防止该数据块的校验数据变得“损坏”，
	从而确保在另一个磁盘发生故障时您仍能成功恢复数据。

	此选项仅适用于在 Linux 中格式化为 Btrfs、Bcachefs 或 ZFS 文件系统，
	以及在 Windows 中格式化为 NTFS 的数据磁盘。
	校验磁盘或使用其他文件系统的数据磁盘将始终使用文件系统的活动版本。

	快照的创建和删除需要管理权限。
	启用此选项时，请确保以必要的权限（例如 sudo）运行 SnapRAID。

	有关快照生命周期的详细说明，请参阅 SNAPSHOTS（快照）部分。
  ...
  autosave SIZE_IN_GIGABYTES
	在同步或 scrub 期间，每处理指定数量的 GB 后就自动保存状态。
  ...
  temp_limit TEMPERATURE_CELSIUS
	...
	要执行此分析，SnapRAID 需要一个系统温度的参考基准值。
	它首先尝试从可用的硬件传感器读取。
...
快照 (Snapshots)
	如果配置中启用了快照选项，SnapRAID 将使用文件系统快照来保持操作期间
	读取的数据稳定，即使实时文件系统上的文件随后被修改或删除也是如此。

	快照的管理是完全自动且透明的。
	您可以完全像以前一样继续使用 SnapRAID，而快照提供的
	额外保护完全在后台处理。

	这提供了两个主要好处：

	一致性 - 在运行时间较长的 sync 或 scrub 期间在活动文件系统上修改的文件
		不会导致校验不匹配或操作中止，因为每个创建了快照的文件系统在操作期间
		都保持不变。
	恢复 - 如果从实时文件系统中更新或删除了一个文件，
		它仍会保留在快照中。如果下次 sync 运行之前发生磁盘故障，
		SnapRAID 会使用快照中保留的数据来重建发生故障的磁盘。

		在活动同步过程中发生磁盘故障的情况下，
		SnapRAID 还能自动从上一次校验计算的快照和当前的快照中读取数据。
		这通过提供解校验方程所需的精确数据块访问，
		最大程度地提高了完全恢复的概率，即使这些数据块在同步之间被修改或删除。

		如果没有快照，健康磁盘上更新或删除的文件会导致缺少数据块，
		而这些数据块可能是修复其他损坏磁盘所必需的。

	快照仅为数据磁盘创建，并且仅在底层文件系统支持此功能时创建。
	校验磁盘始终使用实时文件系统。

	目前，Linux 上的 Btrfs、Bcachefs 和 ZFS 以及 Windows 上的 NTFS 支持此功能。

	您可以混合使用具有不同文件系统的数据磁盘。只有那些
	支持快照的磁盘才会利用它们，而其他磁盘将继续直接在实时文件系统上运行。

	快照的创建和删除需要管理权限。
	启用快照时，请确保以必要的权限（例如 sudo）运行 SnapRAID。

  快照的命令行为
	`sync` 和 `scrub` 命令对支持快照的文件系统的数据磁盘使用快照。
	在启用了快照的磁盘上，该命令看到的是一个不受实时文件系统并发更改影响的
	稳定文件系统镜像。
	在 `sync` 期间，会为每个受支持的数据文件系统独立创建新的快照。
	这些快照共同提供用于校验计算的数据。
	相比之下，`scrub` 命令利用上一个快照（即在最近一次同步期间创建的快照）
	来维持与现有校验匹配的可靠参考点。

	对于 `check` 和 `fix` 命令，上一个快照的使用
	取决于是否使用 -d, --filter-disk 选项指定了特定磁盘。

	通过 -d 明确选择的磁盘（操作的“目标”）始终使用实时文件系统。
	对于 `fix`，这允许将数据恢复到活动的替代磁盘。对于 `check`，
	它允许在相同条件下模拟 `fix` 操作。

	所有其他数据磁盘（“参考”磁盘）都将通过其快照进行访问。
	这确保了即使您在恢复进行时修改健康磁盘上的文件，
	SnapRAID 也有一个稳定、冻结的参考来求解校验方程。

	如果没有提供 -d 选项，SnapRAID 会假设该操作适用于整个阵列，
	在这种情况下，`check` 和 `fix` 将排他性地使用实时文件系统。

	所有其他命令完全在实时文件系统上运行。

快照生命周期 (Snapshots Lifecycle)
	SnapRAID 在每个数据子卷根目录下的隐藏目录中管理三个特定的快照：
	`stable`（稳定）、`pending`（待定）和 `scan`（扫描）。
	在 Btrfs、Bcachefs 和 NTFS 中使用 `.snapraid/` 目录，
	在 ZFS 中使用标准的 `.zfs/snapshot/`。

	`stable` 快照代表上一次成功完成的 `sync` 的状态，
	包含用于计算当前校验的准确数据。它作为 `scrub`、`check`
	和 `fix` 命令的主要数据源。

	`scan` 快照是在 `sync` 开始时创建的临时镜像，
	用于为扫描和校验计算提供冻结状态。在扫描并保存新的内容状态后，
	它将替换以前的 `pending` 快照。

	`pending` 快照是中断的 `sync` 所使用的镜像。
	在操作成功完成后，以前的 `stable` 快照将被删除，
	并且 `pending` 快照将被提升以接替其作为新的稳定参考。

	如果 `sync` 中断，`pending` 快照将被保留。
	随后的 `scrub`、`check` 和 `fix` 命令将使用此待定快照，
	因为它与 .content 文件中记录的文件系统状态相匹配，即使校验仅部分同步。

	在 `sync` 中断的情况下，`check` 和 `fix` 命令
	还会从 `stable` 快照中读取数据，以检索在同步期间更新或删除的文件的
	数据块。这是可能的，因为 .content 文件在整个同步过程中保留了
	所有修改或删除的文件的元数据。此历史信息仅在同步
	成功完成并保存 .content 文件的最终版本后才会清除。

	如果 `sync` 在中断后重新启动，现有的 pending 快照将被删除，
	并创建一个新快照以捕获实时文件系统的当前状态。
	但是，如果 `sync` 中断，请避免在后续 `sync` 成功完成之前修改、删除、
	替换或移动文件。校验可能仍包含根据中断的同步所使用的快照计算的块。
	以后的同步可能会替换该快照，从而导致中间文件版本无法用于恢复。
...
模式 (Pattern)
	...
	双星号通配符 `**` 仅在与目录分隔符相邻时（前面或后面有斜杠）才能跨越
	目录分隔符进行匹配。在此形式下，它可以跨越多个目录层级匹配零个或多个字符。
	例如，`foo/**/bar` 匹配 `foo/bar`、`foo/a/bar` 和 `foo/a/b/bar`；
	`**/build` 匹配 `build` 和 `foo/bar/build`；`src/**.js` 匹配 `src/main.js`
	和 `src/ui/components/main.js`。
	单独的形式 `/**/` 不被接受为完整的模式。

	当 `**` 不与斜杠相邻时，其行为类似于 `*`，不会匹配目录分隔符。例如，
	`**.jpg` 匹配当前目录中的 `photo.jpg`，但不会匹配 `dir/photo.jpg`。
	如需匹配子目录中的文件，请使用 `**/*.jpg`。
...
内容 (Content)
	...
	content 文件由 `sync`、`scrub`、`touch` 和 `rehash` 命令读取和写入。
	它由 `status`、`diff`、`check`、`fix`、`list`、`dup`、`locate` 和
...
已知问题 (Known Issues)
  文件系统边界
	SnapRAID 在扫描数据磁盘时不会跨越文件系统、挂载点或子卷边界。
	当检测到此类边界时，SnapRAID 会报告警告（WARNING）并且不会扫描其范围之外的内容。

	位于此类边界之外的文件不包含在数据磁盘中，如果需要保护它们，则必须单独配置。

  带有文件系统快照的挂载点
	启用文件系统快照时，不支持数据磁盘内的嵌套挂载点和 bind 挂载。

	挂载点属于活动系统的挂载命名空间，不会保存在文件系统快照中。
	因此，在扫描快照时，活动文件系统中被挂载点隐藏的目录可能会变为可见。

	启用文件系统快照时，请勿在数据磁盘内放置嵌套挂载点或 bind 挂载。

  带有文件系统快照的多套配置
	使用相同文件系统快照命名空间的多套独立 SnapRAID 配置无法安全共享快照支持。

	在 Btrfs 和 Bcachefs 上，只有一套启用了快照支持的 SnapRAID 配置可以使用
	同一子卷内的数据目录。在 ZFS 上，相同的限制适用于数据集，在 NTFS 上适用于卷。

	共享相同快照命名空间的多套配置使用相同的 `stable`、`pending` 和 `scan` 快照。
	因此，运行其中一套配置可能会替换或删除另一套配置所需的恢复快照。

  未更改时间戳的原地文件修改
	SnapRAID 通过比较文件大小和修改时间戳（mtime）来检测文件修改。

	如果应用程序在不更改文件大小或不更新其修改时间戳的情况下原地修改文件，
	则 “diff” 和 “sync” 将不会检测到文件已更改，并且其奇偶校验也不会更新。

	为确保 SnapRAID 识别此类更改，请更新文件的修改时间戳（例如使用 “touch” 实用程序）。
	否则，该差异仅在后续的 “scrub” 或 “check” 期间作为哈希不匹配被检测到。

  Windows 非 ASCII 大小写匹配
	SnapRAID 在所有平台上均使用标准且可预测的 UTF-8 编码，仅对 ASCII 字符进行
	不区分大小写的匹配。

	Windows 文件系统受到遗留设计的困扰，即通过任意的、依赖于系统的大小写折叠表在
	Unicode 文件名之间强制实行不区分大小写。因此，Windows 掩盖了文件系统的真实情况，
	将完全不同的 Unicode 字节序列视为同一个文件。

	例如，`À.txt` 和 `à.txt` 是不同的 Unicode 名称，但 Windows 却不合逻辑地将它们
	视为相同文件，而 SnapRAID 则保留了它们的真实区别。

	该限制是由于 Windows 对 Unicode 文件名的不一致处理及其对不区分大小写别名的
	执着所致。为了防止因 Windows 的大小写折叠而产生差异，请在配置路径和过滤器中
	始终为非 ASCII 字符指定准确的大小写。

  部分 fix 恢复
	当使用 `-S, --start` 或 `-B, --count` 限制 `fix` 时，
	SnapRAID 仅对选定的块进行操作，而不执行文件级别的最终化处理或隔离。

	如果恢复产生了无法证实代表当前内容的尽力而为（best-effort）数据，
	此类块仍可能以常规文件名写入。与这些已恢复块相关的不确定性在命令终止后
	不会保留。

	因此，后续的 `fix` 会将这些可读取的块视为有效的文件数据，并且不会尝试
	再次恢复它们。这可能导致后续恢复报告成功，但在文件中留下不完整或过时的数据。

	`-S` 和 `-B` 选项仅适用于高级手动恢复，通常在常规 `fix` 失败后使用。
	如果部分 `fix` 报告了无法恢复的数据，请在尝试另一次恢复之前删除受影响的
	文件或对其重命名，或者手动验证其内容。

  NTFS 硬链接与排除的路径
	在 NTFS 上，目录条目会为每个硬链接单独缓存文件大小和时间戳。
	当通过其中一个链接修改文件时，指向同一文件的其他链接的目录条目
	可能会保留过时的元数据，直到它们被访问为止。

	如果在扫描过程中遇到同一文件的至少两个硬链接，SnapRAID 会自动
	检测并规范化这种情况。但是，如果仅扫描了一个硬链接，而其他硬链接
	被过滤器排除或位于数据磁盘路径之外，SnapRAID 将无法检测到 inode 冲突，
	并会依赖单个可见链接的缓存元数据。

	因此，通过被排除或外部的硬链接所做的修改，在操作系统刷新所包含链接
	的元数据之前（例如通过该路径访问或打开文件时），可能会被 SnapRAID 忽略。

	为避免此限制，请避免通过已从 SnapRAID 中排除的硬链接来修改受保护
	的文件，或者确保文件的所有硬链接都包含在扫描中，以便 SnapRAID 可以
	对它们进行规范化。

  中断的多重奇偶校验 sync 后的恢复
	在具有多个奇偶校验磁盘的阵列中，中断的 `sync` 可能会使不同奇偶校验
	级别之间暂时彼此不同步。例如，奇偶校验 1 可能已经包含新更新的数据，
	而奇偶校验 2 仍包含旧数据。

	在执行 `check` 或 `fix` 期间，SnapRAID 会尝试不同的奇偶校验组合，
	并根据已知的块哈希（覆盖整个块，通常为 256 KiB）验证重建的数据。

	如果 SnapRAID 尝试使用新旧奇偶校验的不一致混合进行重建，线性 RAID
	方程将产生不正确的数据。要错误地接受这种无效组合，这些方程必须在同一
	条带中的其他未同步块被重建为错误数据的情况下，巧合地完美重建出至少一个
	完整的 256 KiB 块以使其哈希完全匹配。

	这种情况并不依赖于哈希碰撞；相反，它需要混合奇偶校验方程本身碰巧
	生成原始的完整 256 KiB 块内容。虽然在特别构造的数据或高度重复的数据
	情况下在数学上是可能的，但在实际数据中这几乎是不可能的。

	如果所有磁盘都运行良好，在进行进一步更改之前完成中断的 `sync` 可以消除
	这种不确定性。如果磁盘发生故障且需要恢复，在运行 `check` 或 `fix` 之前
	请避免修改任何数据磁盘。

  保留的文件和目录名称
	SnapRAID 保留了一些文件、目录和快照名称用于其内部操作。请勿将这些名称
	用于用户数据：

	.snapraidignore - 本地排除规则文件。
	.snapraid - 快照容器目录。
	snapraid-* - 由 SnapRAID 管理的 ZFS 快照名称。
	*.unrecoverable - 在恢复过程中被隔离的受损文件。
	CONTENT.tmp - 用于 content 更新的临时文件。
	CONTENT.lock - content 锁定文件。
	PARITY.spaceholder - 在 Windows 上分配奇偶校验期间使用的临时占位文件。

	将这些名称用于无关数据可能会导致文件或快照被忽略、被误认为是元数据、
	被覆盖、重命名或删除。

	在 Windows 上，所有大小写变体也都被保留，即使在配置为区分大小写的 NTFS
	目录中也是如此（例如 .SNAPRAIDIGNORE、.SnapRaid、CONTENT.TMP 和
	CONTENT.Lock）。
...
版权 (Copyright)
	此文件版权所有 (C) 2026 Andrea Mazzoleni
