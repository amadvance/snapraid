...
已知问题 (Known Issues)
  ...
  无法恢复的文件后缀
	当 `fix` 无法完全恢复损坏的文件时，它会通过在文件名后附加 `.unrecoverable`
	扩展名来对其进行隔离。

	如果后续的 `fix` 重试恢复，它将在常规文件名下创建一个新文件，并在恢复进行
	期间保留旧的 `.unrecoverable` 文件。如果重试失败，新的失败结果将替换旧的
	隔离文件。如果重试完全成功，SnapRAID 将删除已废弃的 `.unrecoverable` 文件。
	隔离文件绝不会被恢复为常规文件名。

	由于 SnapRAID 仅根据文件名识别隔离文件，因此任何碰巧使用 `.unrecoverable`
	后缀的预先存在的文件，如果其基本名称与正在修复的文件匹配，都可能会被截断、
	覆盖、替换或删除。

	请避免对常规文件使用 `.unrecoverable` 扩展名，并确保在配置中包含
	`exclude *.unrecoverable`，以防止在奇偶校验数据集中跟踪此类文件。
