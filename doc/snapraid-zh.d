...
Pattern
	...
	=FILE
		选择任何名为 FILE 的文件。您可以使用任何 globbing
		字符，如 * 和 ?，以及字符类，如 [a-z]。
		此模式仅适用于文件，不适用于目录。

	=DIR/
		选择任何名为 DIR 的目录及其内部的所有内容。
		您可以使用任何 globbing 字符，如 * 和 ?。
		此模式仅适用于目录，不适用于文件。

	=/PATH/FILE
		选择完全指定的 file path。您可以使用任何
		globbing 字符，如 * 和 ?，但它们绝不匹配
		目录斜杠。
		此模式仅适用于文件，不适用于目录。

	=/PATH/DIR/
		选择完全指定的 directory path 及其内部的所有内容。
		您可以使用任何 globbing 字符，如 * 和 ?，但
		它们绝不匹配目录斜杠。
		此模式仅适用于目录，不适用于文件。
