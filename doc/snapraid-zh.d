...
配置 (Configuration)
  ...
  snapshot
	...
	这确保 historical 了文件在特定时间点的一致视图，从而防止由并发文件修改引起的错误。
...
快照 (Snapshots)
	...
	一致性 - 在运行时间较长的 sync 或 scrub 期间在活动文件系统上修改的文件
		不会导致校验不匹配或操作中止，因为 SnapRAID 看到的是一个冻结的时间点图像。
  ...
  快照的命令行为
	...
	这确保 historical 了即使您在恢复进行时修改健康磁盘上的文件，
...
快照生命周期 (Snapshots Lifecycle)
	...
	`pending` 快照是在 `sync` 开始时创建 interim 的临时图像，
