...
已知问题 (Known Issues)
  ...
  硬链接
	SnapRAID 将一个硬链接标识为原始文件，并将其他硬链接表示为对它的引用。
	选择哪个链接作为原始文件取决于扫描目录条目的顺序。

	如果此顺序发生更改（例如在重命名、恢复文件或重新创建目录之后），
	即使链接的文件仍然表示相同的数据，“diff” 也可能会报告表面上的移动、更新、
	添加或删除。

	这并不表示数据损坏，但报告的差异可能会令人困惑，并且可能与在文件系统上
	执行的操作不直接对应。
