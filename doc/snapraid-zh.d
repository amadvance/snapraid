...
配置 (Configuration)
  ...
  snapshot
	...
	启用后，SnapRAID 会在“sync”开始时创建数据的只读快照。
	这确保了文件在特定时间点的一致视图，从而防止由并发文件修改引起的错误。
...
快照 (Snapshots)
	...
	一致性 - 在运行时间较长的 sync 或 scrub 期间在活动文件系统上修改的文件
		不会导致校验不匹配或操作中止，因为 SnapRAID 看到的是一个冻结的时间点镜像。
  ...
  快照的命令行为
	`sync` 和 `scrub` 命令都完全在快照上运行，
	以确保所有数据操作都是针对一致的、冻结的状态进行的。通过利用这些快照，
	SnapRAID 阻止了本由实时文件系统上的并发文件修改所触发的校验不匹配。
	在 `sync` 期间，该命令使用在过程开始时创建的新快照来捕获用于校验计算的当前状态。
...
快照生命周期 (Snapshots Lifecycle)
	...
	`pending` 快照是在 `sync` 开始时创建的临时镜像，
	用于为校验计算提供冻结状态。在操作成功完成后，
	以前的 `stable` 快照将被删除，并且 `pending` 快照将被提升
	以接替其作为新的稳定参考。
