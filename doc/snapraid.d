...
Getting Started
  ...
  Pooling
	...
	the SnapRAID community. Mergerfs provides a more flexible and efficient
...
Configuration
  ...
  smartctl DISK/PARITY OPTIONS...
	...
	You can also specify a custom info options string using the
	`[info: ...]` tag to override the default `-a` option.
...
Pattern
	...
	The double star `**` wildcard matches across directory separators only
	when it is adjacent to a directory separator (preceded or followed by a
	slash). In this form, it matches zero or more characters across directory
	levels. For example, `foo/**/bar` matches `foo/bar`, `foo/a/bar`, and
	`foo/a/b/bar`; `**/build` matches `build` and `foo/bar/build`; and
	`src/**.js` matches `src/main.js` and `src/ui/components/main.js`.
	The standalone form `/**/` is not accepted as a complete pattern.

	When `**` is not adjacent to a slash, it behaves like `*` and does not
	match directory separators. For example, `**.jpg` matches `photo.jpg`
	within the current directory, but does not match `dir/photo.jpg`. Use
	`**/*.jpg` for that purpose.
	...
	component, while `**` can match across directory separators only when
	adjacent to a slash.
...
Known Issues
  Filesystem boundaries
	SnapRAID does not cross filesystem, mount, or subvolume boundaries while
	scanning a data disk. When such a boundary is detected, SnapRAID reports
	a WARNING and does not scan beyond it.

	Files located beyond such boundaries are not included in the data disk and
	must be configured separately if they have to be protected.

  Mount points with filesystem snapshots
	When filesystem snapshots are enabled, nested mount points and bind mounts
	inside a data disk are not supported.

	Mount points are part of the live system mount namespace and are not
	preserved in filesystem snapshots. Consequently, when scanning a snapshot,
	a directory hidden by a mount point in the live filesystem may become
	visible.
  ...
  In-place file modifications without timestamp changes
	...
	If an application modifies a file in-place without altering its size
	or updating its modification timestamp, "diff" and "sync" will not
	detect that the file has changed, and its parity will not be updated.

	To ensure SnapRAID recognizes such changes, update the file's
	modification timestamp (for example, with the "touch" utility).
	Otherwise, the difference will only be detected as a hash mismatch
	during a subsequent "scrub" or "check".

  Windows non-ASCII case matching
	SnapRAID uses standard, predictable UTF-8 encoding across all platforms,
	limiting case-insensitive matching to ASCII characters.

	Windows filesystems suffer from the legacy design of enforcing case
	insensitivity across Unicode filenames via arbitrary, system-dependent
	folding tables. Consequently, Windows obscures the reality of the
	filesystem by treating completely different Unicode byte sequences as if
	they were the same file.

	For example, `À.txt` and `à.txt` are distinct Unicode names, yet Windows
	illogically treats them as identical, while SnapRAID preserves their true
	distinction.

	This limitation is caused by Windows's inconsistent handling of Unicode
	filenames and its insistence on case-insensitive aliasing. To prevent
  ...
  Recovery after an interrupted multi-parity sync
	In an array with multiple parity disks, an interrupted `sync` can leave
	parity levels temporarily out of sync with each other. For example,
	parity 1 might already contain newly updated data, while parity 2 still
	contains the old data.

	During `check` or `fix`, SnapRAID tries different parity combinations
	and validates reconstructed data against the known block hashes (covering
	the full block, typically 256 KiB).

	If SnapRAID attempts a reconstruction using an inconsistent mix of old
	and new parity, the linear RAID equations will produce incorrect data.
	For this invalid combination to be falsely accepted, the equations would
	have to accidentally reconstruct at least one complete 256 KiB block so
	perfectly that its hash matches, while other unsynchronized blocks in the
	same stripe are reconstructed with incorrect data.

	This scenario does not depend on a hash collision; rather, the mixed
	parity equations themselves would have to coincidentally yield the exact
	original 256 KiB block contents. While mathematically possible with
	specially crafted or highly repetitive data, it is practically impossible
	with real-world data.

	If all disks are healthy, completing the interrupted `sync` before making
	further changes removes this ambiguity. If a disk has failed and recovery
	is required, avoid modifying any data disks before running `check` or
	`fix`.

  Reserved file and directory names
	SnapRAID reserves some file, directory, and snapshot names for its internal
	operation. Do not use these names for user data:

	.snapraidignore - Local exclusion-rules file.
	.snapraid - Snapshot container directory.
	snapraid-* - SnapRAID-managed ZFS snapshot names.
	*.unrecoverable - Damaged files quarantined during recovery.
	CONTENT.tmp - Temporary file for content updates.
	CONTENT.lock - Content lock file.
	PARITY.spaceholder - Temporary space-holder file during Windows parity
		allocation.

	Using these names for unrelated data may cause files or snapshots to be
	ignored, misinterpreted as metadata, overwritten, renamed, or deleted.

	On Windows, all case variants are reserved as well, even in NTFS directories
	configured for case-sensitive operation (for example, .SNAPRAIDIGNORE,
	.SnapRaid, CONTENT.TMP, and CONTENT.Lock).
