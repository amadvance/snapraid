...
Limitazioni (Limitations)
	...
	* Con il supporto snapshot (disponibile su Btrfs, ZFS, Bcachefs e NTFS),
		SnapRAID può mantenere automaticamente un riferimento congelato
		indipendente di ogni file system dati supportato.
...
Comandi (Commands)
  ...
  smart
	...
	Altre possibili stringhe di stato sono:
		logfail - In passato, alcuni attributi erano inferiori alla
			soglia.
		logerr - Il log degli errori del dispositivo contiene errori.
		selferr - Il log dell'autotest del dispositivo contiene errori.
  ...
  diff
	...
	riepilogo delle modifiche, raggruppate per:
		equal - File invariati rispetto a prima.
		added - File aggiunti che non erano presenti prima.
		removed - File rimossi.
		updated - File con dimensioni o timestamp diversi, il che
			significa che sono stati modificati.
		moved - File spostati o rinominati sullo stesso disco mantenendo
			lo stesso inode. Sono identificati dall'avere lo stesso inode,
			dimensione e timestamp, ma un percorso diverso. Anche il nome
			del file può essere diverso.
		...
		relocated - File spostati in una directory diversa o su un altro disco
			dove il file originale è scomparso e l'inode non è mantenuto.
			Sono identificati dall'avere lo stesso nome, dimensione e timestamp.
			Se il timestamp al sottomultiplo del secondo è zero, il percorso
			completo deve corrispondere per essere identificato. A differenza
			dei file 'moved', i file ricollocati richiedono che il nome del
			file rimanga identico.
  ...
  scrub
	...
	0-100 - Esegue lo scrub della percentuale specificata di blocchi.
...
Opzioni (Options)
	...
	-S, --start BLKSTART
		...
		ed è supportata solo da `check` e `fix`.
...
Configurazione (Configuration)
  ...
  snapshot
	...
	Quando abilitato, SnapRAID crea uno snapshot in sola lettura di ogni
	file system dati supportato all'inizio di un 'sync'. Ciò fornisce una
	vista stabile di ciascun file system, impedendo che modifiche successive
	cambino i dati utilizzati per il calcolo della parità.
  ...
  smartctl DISK/PARITY OPTIONS...
	...
	È inoltre possibile specificare una stringa di opzioni di informazioni
	personalizzate tramite il tag `[info: ...]` per sostituire l'opzione
	`-a` predefinita.
  ...
  smartignore DISK/PARITY ATTR [ATTR...]
	...
	È possibile specificare più attributi separati da spazi.
	Ciascun attributo può essere specificato come un numero da 1 a 255, o come
	un nome insensibile al maiuscolo/minuscolo (ad es. `Current_Pending_Sector`).

	Ad esempio, per ignorare gli attributi `Current Pending Sector Count` e
	`Reallocated Sectors Count` su tutti i dischi per numero o nome:

		:smartignore * 197 5
		:smartignore * Current_Pending_Sector Reallocated_Sector_Ct

	Per ignorarli solo sul primo disco di parità:
...
Snapshot (Snapshots)
	...
	utilizza gli snapshot del file system per mantenere stabili i dati letti
	durante un'operazione, anche quando i file sui file system live vengono
	successivamente modificati o eliminati.
	...
	Coerenza - I file modificati sul file system live durante un sync o uno scrub di
		...
		perché ogni file system con snapshot rimane invariato per l'intera durata
		dell'operazione.
  ...
  Comportamento dei comandi con gli snapshot (Command Behavior with Snapshots)
	I comandi `sync` e `scrub` utilizzano snapshot per i dischi dati i cui
	file system li supportano. Su un disco con snapshot, il comando vede
	un'immagine del file system stabile e non influenzata da modifiche
	simultanee sul file system live.
	Durante un `sync`, viene creato un nuovo snapshot in modo indipendente
	per ciascun file system dati supportato. Questi snapshot forniscono
	collettivamente i dati utilizzati per il calcolo della parità.
...
Ciclo di vita degli snapshot (Snapshots Lifecycle)
	SnapRAID gestisce tre snapshot specifici, `stable`, `pending` e `scan`,
	...
	Lo snapshot `scan` è un'immagine temporanea creata all'inizio
	di un `sync` per fornire uno stato congelato per la scansione e il calcolo della parità.
	Dopo la scansione e il salvataggio del nuovo stato del contenuto, sostituisce il precedente
	snapshot `pending`.

	Lo snapshot `pending` è l'immagine utilizzata da un `sync` interrotto.
...
Pattern
	...
	Il punto interrogativo `?` corrisponde a qualsiasi carattere singolo eccetto il
	...
	L'asterisco singolo `*` corrisponde a qualsiasi sequenza di caratteri, ma come il
	...
	Il carattere jolly doppio asterisco `**` corrisponde attraverso i separatori
	di directory solo quando è adiacente a un separatore di directory (preceduto
	o seguito da uno slash). In questa forma, corrisponde a zero o più caratteri
	attraverso i livelli di directory. Per esempio, `foo/**/bar` corrisponde a
	`foo/bar`, `foo/a/bar` e `foo/a/b/bar`; `**/build` corrisponde a `build` e
	`foo/bar/build`; e `src/**.js` corrisponde a `src/main.js` e
	`src/ui/components/main.js`.
	La forma autonoma `/**/` non è accettata come pattern completo.

	Quando `**` non è adiacente a uno slash, si comporta come `*` e non
	corrisponde ai separatori di directory. Per esempio, `**.jpg` corrisponde a
	`photo.jpg` nella directory corrente, ma non corrisponde a `dir/photo.jpg`.
	Usa `**/*.jpg` per questo scopo.

	Le classi di caratteri che usano le parentesi quadre `[]` corrispondono a un singolo carattere da un
	...
	La distinzione fondamentale da ricordare è che `*`, `?` e le classi di caratteri
	rispettano tutti i confini delle directory e corrispondono solo all'interno di un singolo
	componente del percorso, mentre `**` può corrispondere attraverso i separatori di directory
	solo quando è adiacente a uno slash.
...
Problemi noti (Known Issues)
  Limiti del filesystem
	SnapRAID non attraversa i confini di filesystem, punti di mount o subvolume
	durante la scansione di un disco dati. Quando viene rilevato un tale confine,
	SnapRAID segnala un WARNING e non prosegue la scansione al suo interno.

	I file che si trovano oltre tali confini non sono inclusi nel disco dati e
	devono essere configurati separatamente se devono essere protetti.

  Punti di mount con snapshot del filesystem
	Quando le snapshot del filesystem sono abilitate, i punti di mount annidati
	e i bind mount all'interno di un disco dati non sono supportati.

	I punti di mount fanno parte del namespace di mount del sistema in esecuzione
	e non vengono preservati nelle snapshot del filesystem. Di conseguenza,
	durante la scansione di una snapshot, una directory nascosta da un punto di
	mount nel filesystem live potrebbe diventare visibile.
  ...
  Modifiche ai file sul posto senza variazione di data/ora
	...
	Se un'applicazione modifica un file in-place senza alterarne la dimensione
	o aggiornarne il timestamp di modifica, "diff" e "sync" non rileveranno che
	il file è cambiato e la sua parità non verrà aggiornata.

	Per garantire che SnapRAID riconosca tali modifiche, aggiornare il timestamp
	di modifica del file (ad esempio con l'utilità "touch"). Altrimenti, la
  ...
  Recupero con fix parziale
	...
	finalizzazione a livello di file né la quarantena.

	Se il recupero produce dati "best-effort" che non possono essere provati
	come rappresentativi del contenuto corrente, tali blocchi potrebbero
	comunque essere scritti con il normale nome di file. L'incertezza associata
	a questi blocchi recuperati non persiste dopo il termine del comando.

	Di conseguenza, un successivo `fix` tratterà questi blocchi leggibili come
	dati di file validi e non tenterà di recuperarli nuovamente. Ciò può far sì
	che un recupero successivo segnali il successo pur lasciando nel file dati
	incompleti o obsoleti.

	Le opzioni `-S` e `-B` sono destinate esclusivamente al recupero manuale
  ...
  Recupero dopo un sync a parità multipla interrotto
	In un array con più dischi di parità, un `sync` interrotto può lasciare
	i livelli di parità temporaneamente disallineati tra loro. Ad esempio,
	la parità 1 potrebbe già contenere i dati appena aggiornati, mentre la
	parità 2 contiene ancora i vecchi dati.

	Durante `check` o `fix`, SnapRAID prova diverse combinazioni di parità
	e convalida i dati ricostruiti rispetto agli hash dei blocchi noti
	(che coprono l'intero blocco, tipicamente 256 KiB).

	Se SnapRAID tenta una ricostruzione utilizzando una combinazione
	incoerente di parità vecchia e nuova, le equazioni lineari del RAID
	produrranno dati errati. Affinché questa combinazione non valida venga
	accettata erroneamente, le equazioni dovrebbero accidentalmente
	ricostruire almeno un blocco completo da 256 KiB in modo così perfetto
	da far corrispondere il suo hash, mentre altri blocchi non sincronizzati
	nella stessa stripe vengono ricostruiti con dati errati.

	Questo scenario non dipende da una collisione di hash; piuttosto, le
	equazioni di parità mista dovrebbero produrre casualmente l'esatto
	contenuto originale del blocco da 256 KiB. Sebbene matematicamente
	possibile con dati appositamente creati o fortemente ripetitivi, è
	praticamente impossibile con dati reali.

	Se tutti i dischi sono integri, completare il `sync` interrotto prima
	di apportare ulteriori modifiche rimuove questa ambiguità. Se un disco
	si è guastato ed è necessario il recupero, evitare di modificare i dischi
	dati prima di eseguire `check` o `fix`.

  Nomi di file e directory riservati
	SnapRAID riserva alcuni nomi di file, directory e snapshot per le proprie
	operazioni interne. Non usare questi nomi per i dati utente:

	.snapraidignore - File locale delle regole di esclusione.
	.snapraid - Directory contenitore degli snapshot.
	snapraid-* - Nomi degli snapshot ZFS gestiti da SnapRAID.
	*.unrecoverable - File danneggiati messi in quarantena durante il ripristino.
	CONTENT.tmp - File temporaneo per gli aggiornamenti del content.
	CONTENT.lock - File di blocco per il content.
	PARITY.spaceholder - File segnaposto temporaneo durante l'allocazione
		della parità su Windows.

	L'uso di questi nomi per dati non correlati può causare l'ignoramento,
	l'errata interpretazione come metadati, la sovrascrittura, la ridenominazione
	o l'eliminazione di file o snapshot.

	Su Windows, tutte le varianti di maiuscole e minuscole sono anch'esse
	riservate, anche nelle directory NTFS configurate per il funzionamento con
	distinzione tra maiuscole e minuscole (ad esempio, .SNAPRAIDIGNORE,
	.SnapRaid, CONTENT.TMP e CONTENT.Lock).
