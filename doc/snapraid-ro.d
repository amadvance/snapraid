...
Model (Pattern)
	Modelele sunt utilizate pentru a selecta un subset de fișiere de exclus sau inclus în
	proces.

	Există patru tipuri diferite de modele:

	=FILE
		Selectează orice fișier numit FILE. Puteți utiliza orice caracter de globbing
		precum * și ?, și clase de caractere precum [a-z].
		Acest model se aplică numai fișierelor, nu directoarelor.

	=DIR/
		Selectează orice director numit DIR și tot ce se află în interior.
		Puteți utiliza orice caracter de globbing precum * și ?.
		Acest model se aplică numai directoarelor, nu fișierelor.

	=/PATH/FILE
		Selectează calea exactă a fișierului specificat. Puteți utiliza orice
		caractere de globbing precum * și ?, dar acestea nu se potrivesc niciodată cu o
		bară oblică de director (directory slash).
		Acest model se aplică numai fișierelor, nu directoarelor.

	=/PATH/DIR/
		Selectează calea exactă a directorului specificat și tot ce se află
		în interior. Puteți utiliza orice caractere de globbing precum * și ?, dar
		acestea nu se potrivesc niciodată cu o bară oblică de director.
		Acest model se aplică numai directoarelor, nu fișierelor.
