...
static void state_map(struct snapraid_state* state)
{
	...
	if (!state->opt.no_warnings) {
		...
		if (diskcount >= 36 && state->level < 6) {
			log_info(EUSER, "WARNING! For %zu disks, it's recommended to use six parity levels.\n", diskcount);
		} else if (diskcount >= 29 && state->level < 5) {
			log_info(EUSER, "WARNING! For %zu disks, it's recommended to use five parity levels.\n", diskcount);
		} else if (diskcount >= 22 && state->level < 4) {
			log_info(EUSER, "WARNING! For %zu disks, it's recommended to use four parity levels.\n", diskcount);
		} else if (diskcount >= 15 && state->level < 3) {
			log_info(EUSER, "WARNING! For %zu disks, it's recommended to use three parity levels.\n", diskcount);
		} else if (diskcount >= 5 && state->level < 2) {
			log_info(EUSER, "WARNING! For %zu disks, it's recommended to use two parity levels.\n", diskcount);
		}
		/* LCOV_EXCL_STOP */
	}
}
...
/**
 * Context for reading the content file.
 */
struct state_read_context {
...
{
	struct state_write_context* context = arg;
...
static void state_write_content(struct snapraid_state* state, struct state_write_info* out_info)
{
	...
	unsigned k;
	struct state_write_context* context;
	void* retval;
#endif
	tommy_node* i;
	block_off_t blockmax;
	...
	while (i) {
		...
		context = malloc_nofail(sizeof(struct state_write_context));
	...
	while (i) {
		struct snapraid_content* content = i->data;
		struct state_write_context* context = content->context;
		void* retval;
...
void state_read(struct snapraid_state* state)
{
	...
	/* for each data disk mapping read from the content file */
	map_idx = 0;
	for (node = tommy_list_head(&state->maplist); node != 0 && map_idx < context.count_map; node = node->next, ++map_idx) {
		struct snapraid_map* map = node->data;
	...
	for (l = 0; l < state->level; ++l) {
		...
		for (s = 0; s < state->parity[l].split_mac; ++s) {
			...
			log_tag("content_parity_split:%s:%s:%s:%" PRIi64 "\n",
				parity_name,
				state->parity[l].split_map[s].content_uuid,
