...
/**
 * Three-way partition for quicksort
 * Partitions array into three parts: < pivot, == pivot, > pivot
 * Returns the boundaries of the equal section via lt and gt pointers
 */
static void partition3way(time_t *arr, size_t left, size_t right, size_t *lt, size_t *gt)
{
	/* choose median-of-three as pivot to avoid worst case on sorted data */
	size_t mid = left + (right - left) / 2;
	time_t pivot;

	/* sort left, mid, right and use median as pivot */
	if (arr[left] > arr[mid]) {
		uint64_t tmp = arr[left]; arr[left] = arr[mid]; arr[mid] = tmp;
	}
	if (arr[mid] > arr[right]) {
		uint64_t tmp = arr[mid]; arr[mid] = arr[right]; arr[right] = tmp;
		if (arr[left] > arr[mid]) {
 ...
 */
static void quicksort3way_recursive(time_t *arr, size_t left, size_t right)
{
	if (left >= right)
		return;
	...
	size_t lt, gt;
	partition3way(arr, left, right, &lt, &gt);
	 ...
	 */
	if (lt > left) {
		quicksort3way_recursive(arr, left, lt - 1);
	}
	if (gt < right) {
		quicksort3way_recursive(arr, gt + 1, right);
	}
}
...
void sort_time(time_t *arr, size_t n)
{
	if (n <= 1)
		return;
	quicksort3way_recursive(arr, 0, n - 1);
}
