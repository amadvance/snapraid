...
static void sync_parity_writer(struct snapraid_worker* worker, struct snapraid_task* task)
{
	...
	/* write parity */
	ret = parity_write(parity_handle, blockcur, buffer, state->block_size, state->opt.skip_fallocate);
	if (ret == -1) {
		/* LCOV_EXCL_START */
...
int state_sync(struct snapraid_state* state, block_off_t blockstart, block_off_t blockcount)
{
	...
	for (l = 0; l < state->level; ++l) {
		 ...
		 *
		 * This comparison deliberately does not use physical EOF. It describes
		 * physical I/O extent, not the logical parity layout whose crash-consistency
		 * relationship with the content file must be preserved.
		 */
		...
		parity_physical_size(&parity_handle[l], &out_size);
	 ...
	 * match with another level or the allocated extent. Physical EOF only records
	 * presence; pending or invalid parity is identified independently by block
	 * state and regenerated by sync when required.
	 */
	...
	if (!skip_sync) {
		 ...
		 *
		 * This decision uses the logical parity size, not physical EOF. Physical EOF
		 * is only an I/O extent and does not represent parity validity or the persisted
		 * logical mapping.
		 */
