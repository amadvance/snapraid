...
void cmd_generate_file(const char* path, int size)
{
	...
	if (remove(path) != 0) {
		if (errno != ENOENT) {
			/* LCOV_EXCL_START */
			log_fatal(errno, "Error removing file %s\n", path);
			exit(EXIT_FAILURE);
			/* LCOV_EXCL_STOP */
		}
	} else {
	...
	if (f < 0) {
		/* LCOV_EXCL_START */
		log_fatal(errno, "Error creating file %s\n", path);
		exit(EXIT_FAILURE);
		/* LCOV_EXCL_STOP */
	}
	...
	if (write(f, data, size) != size) {
		/* LCOV_EXCL_START */
		log_fatal(errno, "Error writing file %s\n", path);
		exit(EXIT_FAILURE);
		/* LCOV_EXCL_STOP */
	}
	...
	if (close(f) != 0) {
		/* LCOV_EXCL_START */
		log_fatal(errno, "Error closing file %s\n", path);
		exit(EXIT_FAILURE);
		/* LCOV_EXCL_STOP */
	}
...
void cmd_generate_symlink(const char* path, const char* linkto)
{
	...
	if (symlink(linkto, path) != 0) {
		/* LCOV_EXCL_START */
		log_fatal(errno, "Error writing symlink %s\n", path);
		exit(EXIT_FAILURE);
		/* LCOV_EXCL_STOP */
	}
}
...
void cmd_generate(int disk, int size)
{
	...
	*file = 0;
	...
	if (mkdir(path, 0777) != 0) {
		if (errno != EEXIST) {
			/* LCOV_EXCL_START */
			log_fatal(errno, "Error creating directory %s\n", path);
			exit(EXIT_FAILURE);
			/* LCOV_EXCL_STOP */
		}
	}
...
void cmd_write(const char* path, int size)
{
	...
	if (lstat(path, &st) != 0) {
		/* LCOV_EXCL_START */
		log_fatal(errno, "Error accessing %s\n", path);
		exit(EXIT_FAILURE);
		/* LCOV_EXCL_STOP */
	}
	...
	if (S_ISREG(st.st_mode)) {
		...
		if (lseek(f, off, SEEK_SET) != off) {
			/* LCOV_EXCL_START */
			log_fatal(errno, "Error seeking file %s\n", path);
			exit(EXIT_FAILURE);
			/* LCOV_EXCL_STOP */
		}
...
void cmd_damage(const char* path, int size)
{
	...
	if (S_ISREG(st.st_mode)) {
		...
		if (fallback(f, &st) != 0) {
			/* LCOV_EXCL_START */
			log_fatal(errno, "Error setting time for file %s\n", path);
			exit(EXIT_FAILURE);
			/* LCOV_EXCL_STOP */
		}
...
void cmd_append(const char* path, int size)
{
	...
	if (S_ISREG(st.st_mode)) {
		...
		if (f < 0) {
			/* LCOV_EXCL_START */
			log_fatal(errno, "Error opening file %s\n", path);
			exit(EXIT_FAILURE);
			/* LCOV_EXCL_STOP */
		}
...
void cmd_truncate(const char* path, int size)
{
	...
	if (S_ISREG(st.st_mode)) {
		...
		if (ftruncate(f, off) != 0) {
			/* LCOV_EXCL_START */
			log_fatal(errno, "Error truncating file %s\n", path);
			exit(EXIT_FAILURE);
			/* LCOV_EXCL_STOP */
		}
...
void cmd_delete(const char* path)
{
	if (remove(path) != 0) {
		/* LCOV_EXCL_START */
		log_fatal(errno, "Error removing %s\n", path);
		exit(EXIT_FAILURE);
		/* LCOV_EXCL_STOP */
	}
}
